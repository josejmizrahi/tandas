...
public protocol CheckInRepository: Actor {
...
public actor MockCheckInRepository: CheckInRepository {
    public private(set) var checkIns: [(eventId: UUID, memberId: UUID, method: CheckInMethod)] = []
    public init() {}
    public var nextError: EventError?
    public var alreadyCheckedInIds: Set<UUID> = []  // memberIds already checked

    public func selfCheckIn(eventId: UUID, userId: UUID, locationVerified: Bool) async throws -> RSVP {
        try await record(eventId: eventId, memberId: userId, method: .selfMethod, locationVerified: locationVerified)
    }

    public func hostMarkCheckIn(eventId: UUID, memberId: UUID) async throws -> RSVP {
        try await record(eventId: eventId, memberId: memberId, method: .hostMarked, locationVerified: false)
    }

    public func qrScanCheckIn(eventId: UUID, memberId: UUID) async throws -> RSVP {
...
public actor LiveCheckInRepository: CheckInRepository {
