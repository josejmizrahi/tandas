...
/// implicit AND. §22.4 lifts that limitation: a rule can now express
...
///   ```
///
/// Encoding round-trips back to the compact array form when the tree
/// happens to be a single `.and` of `.leaf` children (the common case).
/// Anything richer encodes as `{op, children}`. That keeps pre-§22.4
/// rules untouched on the wire while enabling tree authoring on top.
public indirect enum ConditionNode: Sendable, Hashable {
    ...
    /// Empty `.and([])` — semantically true (vacuous AND). Used as the
    /// default for "no conditions" rules.
    public static let empty: ConditionNode = .and([])
}

// MARK: - Convenience walkers
...
public extension ConditionNode {
    ...
    /// uses that only care about the SET of conditions (e.g. param
...
extension ConditionNode: Codable {
    ...
    public init(from decoder: Decoder) throws {
        // The legacy wire shape is a JSON array. Try that first so we
        // route pre-§22.4 rules through `.and(.leaf, .leaf, …)` without
        // touching the tree branch.
        ...
        case "or":
            self = .or(children)
        case "not":
            // `not` carries exactly one child. The validator on the
            // server enforces this; the decoder tolerates 0/1/many
            // and picks the first to avoid crashing on stale data.
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        // a JSON array so pre-§22.4 consumers (and the rule_versions
        // snapshot history) see the same shape they always saw.
        ...
        switch self {
        case .leaf(let c):
            // A bare leaf at the root is unusual but representable; emit
            // it as the leaf object directly so the symmetric decoder
            // round-trips.
            var single = encoder.singleValueContainer()
            try single.encode(c)
        case .and(let children):
...
public extension ConditionNode {
    ...
    /// that still pass `[RuleCondition]` (legacy templates, builder
