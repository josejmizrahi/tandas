...
public final class ResourceRulesCoordinator {
    ...
    private func humanize(error: Error) -> String {
        ...
        if raw.contains("only group admins or the event host") {
            ...
            return "Sólo el anfitrión del evento o un fundador pueden crear reglas aquí."
        }
        if raw.contains("only group admins") {
            return "Sólo los fundadores del grupo pueden crear reglas para este recurso."
        }
        if raw.contains("admin only") {
            return "Sólo fundadores pueden crear reglas aquí."
        }
