...
public struct CheckInSection: View {
    public let event: Event
    public let myRSVP: RSVP?
    public let viewerIsHost: Bool
    public let confirmedRSVPs: [RSVP]
    public let memberLookup: (UUID) -> (name: String, avatarURL: URL?)
    public let onSelfCheckIn: () -> Void
    public let onShowQR: () -> Void
    public let onHostMarkCheckIn: (UUID) -> Void

    public init(event: Event, myRSVP: RSVP?, viewerIsHost: Bool, confirmedRSVPs: [RSVP], memberLookup: @escaping (UUID) -> (name: String, avatarURL: URL?), onSelfCheckIn: @escaping () -> Void, onShowQR: @escaping () -> Void, onHostMarkCheckIn: @escaping (UUID) -> Void) {
        self.event = event
        self.myRSVP = myRSVP
        self.viewerIsHost = viewerIsHost
        self.confirmedRSVPs = confirmedRSVPs
        self.memberLookup = memberLookup
        self.onSelfCheckIn = onSelfCheckIn
        self.onShowQR = onShowQR
        self.onHostMarkCheckIn = onHostMarkCheckIn
    }
