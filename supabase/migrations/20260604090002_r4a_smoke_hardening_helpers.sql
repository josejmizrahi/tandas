-- =============================================================================
-- _smoke_r4a_hardening_helpers — verifies R.4A deliverables end-to-end.
-- =============================================================================
-- Asserted cases:
--   C1  trust_edges trigger is registered against touch_updated_at
--   C1b trigger actually fires on UPDATE (sets updated_at to txn now())
--   C2  actors.is_context derived false for person subtype on insert
--   C2b actors.is_context derived true for collective/family via create_context
--   C3  every operational subtype is is_context = true
--   C3b person/system never carry is_context = true
--   C4  is_context_actor(true_context) is true
--   C5  is_context_actor(person) is false
--   C6  is_context_actor(missing uuid) is false
--   C7  current_person_actor_id() matches current_actor_id() under same auth.uid()
--   C8  actor_has_permission delegates to has_actor_authority (both founder and
--       outsider results agree)
--   C9  actor_has_permission self-context shortcut returns true
--   C10 idx_actors_is_context and idx_resources_created_by exist
-- =============================================================================
...
begin
  ...
  -- C2b: collective/family insert via create_context → is_context true
