-- PARTE 12 — N.4 Rules + engine smoke.
--
-- Cobertura:
--   - Catalog: 3 shape_keys necesarias presentes.
--   - validate_rule_shape happy + unknown.
--   - create_engine_rule end-to-end (incluye publish_rule_version internamente).
--   - Engine eval audit: matched=true cuando trigger+condition cumplen.
--   - Engine eval audit: matched=false cuando condition no cumple (audit-only).
--   - consequence.send_notification → notifications_outbox row pending.
--
-- DEFERIDOS a follow-up: N.4.6 dual rule fire, N.4.7 recursion depth=5,
-- N.4.8 idempotency, N.4.9 cultural_norm auto-promote (cubierto por A2 mig).
...
BEGIN
  ...
  -- N.4.1: validate_rule_shape happy path → valid=true.
  ...
  -- N.4.2: validate_rule_shape con shape_key inexistente → valid=false.
  ...
  -- N.4.3: create_engine_rule → row activa + emite rule.created.
  PERFORM set_config('request.jwt.claims', jsonb_build_object('sub', v_user_a::text)::text, true);
  ...
  -- N.4.4: record_expense $100 > $50 threshold → eval matched + outbox row.
  ...
  SELECT count(*) INTO v_eval_high_matched FROM public.group_rule_evaluations gre
   WHERE gre.group_id = v_group_id
     AND gre.rule_id = v_rule_id
     AND gre.matched = true;
  SELECT count(*) INTO v_outbox_high FROM public.notifications_outbox no_
   WHERE no_.group_id = v_group_id AND no_.category = 'rule.send_notification';
  step := 'N.4.4.expense_above_threshold_matched_and_outbox';
  ok := v_eval_high_matched >= 1;
  detail := 'matched_evals=' || v_eval_high_matched || ' outbox_rows=' || v_outbox_high; RETURN NEXT;

  -- N.4.5: record_expense $40 < $50 threshold → eval audit con matched=false.
