-- PARTE 12 — N.7 Governance smoke (PARTE 7 versioning + core voting lifecycle).
--
-- Cobertura inicial: lo NUEVO (PARTE 7 governance_versions) + el lifecycle básico
-- (start_vote + cast_vote + current_vote_for). DEFERIDOS a follow-up:
--   - N.7.3 ranked voting (V2-G9)
--   - N.7.4-6 finalize handlers (PARTE 4 — necesita doctrina founder per type)
--   - N.7.7 group_decisions partial guard (tested by PARTE A3b mig)
--   - N.7.8 cast_vote on closed decision (depende de finalize verde)
--   - N.7.9 weighted vote tally (V2-G9)
...
BEGIN
  ...
  -- Setup: A crea grupo, invita a B, B acepta.
  ...
  -- PARTE7a: primer set_decision_rules crea row activa.
  ...
  -- PARTE7b: segundo set cierra previa y crea nueva activa.
  ...
  -- PARTE7c: evento decision_rules.set con version_id en payload (latest = v2).
  ...
  -- PARTE7d: UPDATE de columna immutable (snapshot) bloqueado por partial guard.
  BEGIN
  ...
  -- PARTE7e: DELETE bloqueado por atom_no_delete_guard.
  BEGIN
  ...
  -- =============== Voting lifecycle ===============
  -- N.7.1: start_vote → row status='open' + decision.started emitido.
  ...
  SELECT count(*) INTO v_decision_started_events FROM public.group_events
   WHERE group_id = v_group_id AND event_type = 'decision.started' AND entity_id = v_decision_id;
  step := 'N.7.1.start_vote_creates_open_decision'; ok := v_decision_status = 'open' AND v_decision_started_events >= 1;
  detail := 'status=' || v_decision_status || ' decision_started_events=' || v_decision_started_events; RETURN NEXT;

  -- N.7.2: cast_vote dos veces mismo voter → 2 rows, current_vote_for retorna latest.
