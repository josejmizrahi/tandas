...
revoke execute on function public.test_hostAssigned_atom_emission(uuid, uuid, uuid) from public, anon, authenticated;

comment on function public.test_hostAssigned_atom_emission(uuid, uuid, uuid) is
  'Mig 00335: sub-transaction diagnostic that exercises on_resource_event_inserted trigger end-to-end and asserts user_action + system_event side effects with valid FK references. Always rolls back via sentinel raise.';;
