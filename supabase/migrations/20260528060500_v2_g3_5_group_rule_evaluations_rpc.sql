-- 20260528060500 — V2-G3 sub-slice 5 (backend half): list RPC para
-- que iOS visibilice el audit log del engine.
--
-- group_rule_evaluations(group_id, limit, before) devuelve cada eval
-- hidratada con rule_title + trigger_event_type + matched_predicate
-- (con outcome real post-G3.4) + actions_emitted (per-action
-- {kind, execution, status, error?}). Active-member gate.
-- p_before es cursor para infinite scroll (paginación por
-- created_at DESC).
