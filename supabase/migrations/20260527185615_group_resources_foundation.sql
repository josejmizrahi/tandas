...
BEGIN
  ...
  RETURN QUERY
  SELECT
    gr.id, gr.group_id, gr.resource_type, gr.name, gr.description, gr.status,
    gr.visibility, gr.ownership_kind, gr.owner_membership_id,
    NULL::uuid AS custodian_membership_id,
    gr.created_by, gr.created_at, gr.updated_at
  ...
  ORDER BY
    CASE gr.resource_type
      WHEN 'fund' THEN 0 WHEN 'space' THEN 1 WHEN 'asset' THEN 2
      WHEN 'document' THEN 3 WHEN 'other' THEN 4 ELSE 9 END,
    gr.name ASC, gr.created_at DESC;
END;
$$;
...
COMMENT ON FUNCTION public.group_resources_active(uuid) IS
  'Primitiva 5 Foundation (mig 20260527040000): active resources for a group, filtered to Foundation types.';

CREATE OR REPLACE FUNCTION public.create_group_resource(
  p_group_id uuid, p_resource_type text, p_name text,
  p_description text DEFAULT NULL, p_visibility text DEFAULT 'members',
...
COMMENT ON FUNCTION public.create_group_resource(uuid, text, text, text, text, text, uuid, uuid) IS
  'Primitiva 5 Foundation (mig 20260527040000): envelope-only create. Foundation types only.';
