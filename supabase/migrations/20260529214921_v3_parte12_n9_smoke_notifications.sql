...
--   - DELETE bloqueado para undispatched.
...
BEGIN
  ...
  -- 0c. PARTE 8 idempotency UNIQUE index presente.
  ...
  -- N.9.2: UPDATE payload bloqueado por partial guard.
  BEGIN
  ...
  -- N.9.2b: UPDATE dispatch_status PERMITIDO (col mutable).
  BEGIN
  ...
  -- N.9.3: DELETE de row recién dispatched (< 30d) bloqueado por retention.
  BEGIN
  ...
  -- N.9.3b: DELETE permitido cuando dispatched_at < now() - 30d (PARTE 8 retention).
  INSERT INTO public.notifications_outbox (group_id, recipient_user_id, category, payload)
  ...
  -- N.9.4: dedup por idempotency_key (PARTE 8 UNIQUE partial).
  INSERT INTO public.notifications_outbox (group_id, recipient_user_id, category, payload)
