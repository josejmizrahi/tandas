-- 20260526113000 — group_members read helper (Foundation Members wiring).
--
-- Founder ask 2026-05-26: iOS Members shell ya está mergeado pero
-- consume mocks. Para conectar la UI a datos reales sin que iOS
-- conozca joins sociales (group_memberships × profiles ×
-- group_member_roles × group_roles), agregamos UN read helper
-- canónico:
--
--   public.group_members(p_group_id) returns table(...)
--
-- iOS llama RPC y recibe filas pre-armadas. Foundation slice ÚNICO
-- propósito: poblar MembersListView. No expone admin actions,
-- realtime, ni roles editables. La forma del row es estable; cualquier
-- extensión (avatar resolución desde Storage, conteo de fines abiertos
-- por miembro, etc.) llega como columnas nuevas en migraciones futuras.
--
-- Statuses visibles en Foundation:
--   - active, invited, requested, suspended
--   - excluidos: left, banned (per founder spec)
--
-- Errores canónicos:
...
BEGIN
  ...
  SELECT
    ...
    -- display_name: profile name > profile username > 'Miembro'
    COALESCE(
  ...
  ORDER BY
    ...
    -- Then status priority (matches MemberSectionKind.renderOrder).
    CASE gm.status
      WHEN 'active'    THEN 0
      ...
      ELSE 4
    END,
    -- Within active, regular members before provisionals.
