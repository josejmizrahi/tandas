-- 20260528030000 — V2-G2 sub-slice 4: metadata jsonb en start_vote +
-- finalize_vote handler para reference_kind='membership'.
--
-- group_decisions ya tiene columna metadata jsonb desde 00001 — pero
-- start_vote la hardcodeaba a '{}' al insertar. Esta migración:
--   1. Agrega p_metadata jsonb DEFAULT NULL a start_vote y lo
--      persiste en group_decisions.metadata (NULL → '{}').
--   2. Extiende finalize_vote para que cuando reference_kind='membership'
--      + outcome=passed lea metadata->>'target_state' y llame
--      set_membership_state(reference_id, target_state, reason).
--      target_state debe ser uno de 'active' / 'suspended' /
--      'expelled' / 'inactive'; un valor faltante o inválido no
--      bloquea el finalize (se registra y se sigue).
--
-- Razón: el switch de finalize_vote ya cubría sanction / dispute /
-- mandate_grant / mandate_revoke / dissolution. membership era el
-- siguiente gap natural — la decisión sabe a quién afecta
-- (reference_id), pero necesita decir a qué estado debe pasar. El
-- backend ya tenía set_membership_state expuesto desde la mig
-- foundation P2.

-- ===========================================================================
-- 1. start_vote: agrega p_metadata
-- ===========================================================================
-- CREATE OR REPLACE no permite cambiar la signature de una función
-- existente; PostgREST necesita un único overload para resolver el
-- payload sin ambigüedad. Tiramos la versión de 14 args (sin
-- p_metadata) antes de declarar la nueva de 15.

DROP FUNCTION IF EXISTS public.start_vote(
  uuid, text, text, text, text, text, timestamptz, timestamptz, numeric, numeric, boolean, text, uuid, jsonb
);
...
REVOKE EXECUTE ON FUNCTION public.start_vote(
  uuid, text, text, text, text, text, timestamptz, timestamptz, numeric, numeric, boolean, text, uuid, jsonb, jsonb
) FROM public, anon;
GRANT  EXECUTE ON FUNCTION public.start_vote(
  uuid, text, text, text, text, text, timestamptz, timestamptz, numeric, numeric, boolean, text, uuid, jsonb, jsonb
) TO authenticated;

-- ===========================================================================
-- 2. finalize_vote: agrega rama membership al switch
-- ===========================================================================
...
begin
  ...
  if v_outcome = 'passed' then
    if v_d.reference_kind = 'sanction' and v_d.reference_id is not null then
      ...
      -- V2-G2 sub-slice 4 — derive target state from metadata. Skips
      -- silently when missing/invalid so the finalize itself never
      -- rolls back on a malformed proposal; the system event records
      -- the gap for audit.
