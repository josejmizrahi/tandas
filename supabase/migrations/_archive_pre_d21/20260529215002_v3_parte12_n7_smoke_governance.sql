...
--
-- Cobertura: lo NUEVO (PARTE 7 governance_versions) + el lifecycle básico
...
--   - N.7.7 group_decisions partial guard (cubierto por mig A3b)
--   - N.7.8 cast_vote on closed decision (depende de finalize verde)
--   - N.7.9 weighted vote tally (V2-G9)
--
-- Notas operativas:
--   - decision_type allowed: proposal | poll | election | budget | rule_change |
--     membership | sanction_appeal | mandate_grant | mandate_revoke | dissolution | other.
--     Uso 'other' (sin handler en finalize_vote).
--   - cast_vote tiene 2 overloads (4-arg legacy + 5-arg con p_weight DEFAULT NULL).
--     Llamo el 5-arg explícito (1::numeric) para desambiguar.
--     TODO: drop overload 4-arg en mig separada.
--   - start_vote emite 'decision.proposed' (NO 'decision.started' como decía spec).
--     Founder decisión 2026-05-29: código source of truth, corregir spec.
...
BEGIN
  ...
  v_decision_id := public.start_vote(
    v_group_id, 'Smoke decision 1', 'body', 'other', 'majority', 'majority',
  ...
  v_vote_1_id := public.cast_vote(v_decision_id, NULL::uuid, 'yes'::text, 'first'::text, 1::numeric);
  v_vote_2_id := public.cast_vote(v_decision_id, NULL::uuid, 'no'::text,  'changed_mind'::text, 1::numeric);
