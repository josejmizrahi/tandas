...
--   - group_events_recent retorna feed con eventos del grupo (count + uuid presence).
--
-- Drifts capturados:
--   - group_events_recent retorna `event_uuid` (NO `id` como decía spec §N.8.3).
--   - ORDER BY occurred_at DESC SIN tie-break secundario `id DESC` → ordering
--     no determinístico cuando timestamps coinciden (same-tx). TODO mig separada
--     para agregar tie-break y estabilizar la cursor pagination.
...
BEGIN
  ...
  -- N.8.4: ORDER BY occurred_at DESC sin tie-break ⇒ ordering interno
  -- no determinístico cuando timestamps coinciden (same-tx). Aserto débil
  -- pero estable: count >= 2 + ambos uuids del par emitido están en el feed.
