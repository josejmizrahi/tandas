-- 20260527236000 — V3-A2: register_my_notification_token RPC.
--
-- Why
-- ===
-- iOS needs a canonical surface to deposit APNs device tokens after
-- `registerForRemoteNotifications` succeeds. The table already exists
-- (`public.notification_tokens`, mig 00012) and its RLS allows the
-- caller to write their own row, but the canonical-RPC doctrine for
-- this codebase says iOS does not write tables directly — every
-- mutation goes through a SECURITY DEFINER RPC so the row shape +
-- guards live with the schema.
--
-- What
-- ====
-- One SECURITY DEFINER RPC that upserts on the unique
-- `(user_id, token)` index. Re-registration of the same token bumps
-- `updated_at` so the dispatcher's eventual cleanup pass has a clear
-- "last seen" signal.
--
-- Idempotent and safe to call on every cold launch + every auth
-- state transition.
