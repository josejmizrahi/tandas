...
--   - grant_mandate happy path + event mandate.granted.
--   - grant_mandate validation: ends_at en pasado → raises 22023.
--   - revoke_mandate → status='revoked' + event mandate.revoked.
...
BEGIN
  ...
  -- Cross-group: nuevo mandato vivo en group_x usado en group_y → guard raises.
  v_mandate_id := public.grant_mandate(
    v_group_x, v_membership_b_x, 'spend', 'group', NULL,
