-- PARTE 12 fix: _smoke_money_flow seguía asignando invite_member a uuid escalar,
-- pero invite_member ahora retorna TABLE(invite_id, code, placeholder_membership_id)
-- post-PARTE INV. Adapto las llamadas a SELECT INTO sobre el composite.
-- Resto del flow + asserts intactos.
...
begin
  ...
  -- PARTE 12 fix: invite_member retorna TABLE(invite_id, code, placeholder_membership_id).
  select im.invite_id, im.code into v_invite_id, v_invite_b_code
    from public.invite_member(v_group_id, 'smoke-b@test', null, 'member', null) im;
  ...
  select im.invite_id, im.code into v_invite_c_id, v_invite_c_code
    from public.invite_member(v_group_id, 'smoke-c@test', null, 'member', null) im;
