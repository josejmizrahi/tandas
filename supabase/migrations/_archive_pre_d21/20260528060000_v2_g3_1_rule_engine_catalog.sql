-- 20260528060000 — V2-G3 sub-slice 1: rule engine atom catalog +
-- dry-run validator + engine create wrapper.
--
-- Lockeo institucional desde día 1: el catálogo es atom-based — una
-- fila por trigger (WHEN), condition (IF) o consequence (THEN).
-- El check existente en rule_shapes_catalog.category exige esos tres
-- valores. iOS sólo elige atoms del catálogo (templates-only en G3.1);
-- freeform jsonb es V3.
--
-- Sembrado de 9 atoms mínimos confirmados por founder (3+3+3).
-- list_rule_shapes() expone el catálogo a iOS.
-- validate_rule_shape(jsonb) hace dry-run server-side antes de save.
-- create_engine_rule(...) propose+publish atómico (re-valida server-
-- side, requiere rules.publish — consistencia con publish_rule_version).
-- group_rules_engine(group_id) expone las reglas con engine al iOS
-- hidratadas con condition_tree + consequences para UX explicable.
