...
--
-- Conecta Primitiva 20 (Cultura) con Primitiva 4 (Reglas):
-- una norm endorsed puede promoverse a rule formal en una sola
-- operación atómica. El RPC:
--   1) Lee la norm (group_id, title, body).
--   2) Crea group_rules (status='active') + group_rule_versions
--      (version=1, execution_mode='text') siguiendo el patrón de
--      create_text_rule.
--   3) Retira la norm (status='retired') con
--      metadata={promoted_to_rule_id} en el system event.
--   4) Emite cultural_norm.promoted_to_rule.
--
-- Permission: gated por rules.create. La retirada va embebida
-- (bypassing group.update intencional) porque la consecuencia es
-- automática y derivada del rule.create, no una decisión
-- configuracional separada.
...
BEGIN
  ...
  -- Active-member gate (mirror retire_cultural_norm's check).
  IF NOT EXISTS (
  ...
  -- The body of a norm is optional in the canonical schema but a
  -- rule body is required. Fall back to the title when body is
  -- empty so we never insert an empty rule version.
  v_norm_body := NULLIF(btrim(coalesce(v_norm_body, '')), '');
  IF v_norm_body IS NULL THEN
    v_norm_body := v_norm_title;
  END IF;

  -- Single gate: rule creation. The retire side is a derived
  -- consequence, not a separate configurational decision.
