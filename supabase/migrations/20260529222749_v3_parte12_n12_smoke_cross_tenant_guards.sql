...
BEGIN
  ...
  -- A crea group_x. Sólo A es active.
  ...
  -- B crea group_y. Sólo B es active.
  ...
  -- A (no member de group_y) intenta operar sobre group_y en 5 RPCs.
  PERFORM set_config('request.jwt.claims', jsonb_build_object('sub', v_user_a::text)::text, true);

  -- N.12.1: record_expense en group_y → raises.
  BEGIN
  ...
  -- N.12.2: group_events_recent en group_y → raises.
  BEGIN
  ...
  -- N.12.3: invite_member a group_y → raises.
  BEGIN
  ...
  -- N.12.4: set_decision_rules en group_y → raises (assert_permission group.update).
  BEGIN
  ...
  -- N.12.5: assert_permission en group_y → raises (no membership).
  BEGIN
