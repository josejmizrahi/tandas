-- 20260528060400 — V2-G3 sub-slice 2: cablear evaluate_rules_for_event
-- post-commit en las RPCs domain que aún no lo invocaban.
--
-- Sorpresa al levantar la sub-slice: los 3 money RPCs (record_expense,
-- record_settlement, record_pool_charge) ya estaban cableados desde
-- migrations anteriores. Scope real = 4 callsites canónicos:
--   - issue_sanction        → emite sanction.issued
--   - set_membership_state  → emite member.state_changed (cubre leave_group)
--   - open_dispute          → emite dispute.opened       (cubre dispute_sanction)
--   - finalize_vote         → emite decision.finalized
--
-- Patrón uniforme: capturar uuid_id del record_system_event que ya
-- existía en cada función + invocar evaluator con ese uuid. Cada
-- función gana un `v_event_uuid uuid;` en DECLARE y dos líneas
-- (SELECT INTO + PERFORM) reemplazan el PERFORM record_system_event.
--
-- Recursion: issue_sanction + set_membership_state son tanto entry
-- points como handlers del dispatcher (G3.4). El depth guard + cycle
-- detection del evaluator (G3.3) contienen la cadena. Hoy ninguno de
-- sanction.issued / dispute.opened / member.state_changed / decision.finalized
-- tiene atom-trigger seeded en el catálogo, así que el hook se ejecuta
-- pero no matchea reglas — audit-only hasta que founders publiquen
-- reglas para esos eventos. La ÚNICA cadena viva hoy es:
-- record_expense → engine → issue_sanction (handler) → engine no-op.
--
-- finalize_vote tiene record_system_event intermedios para
-- rule.archived/rule.activated/money.pool_charge_created adentro de
-- los outcome handlers G2; esos NO se cablean porque son bookkeeping
-- side effects de la decisión, no eventos por los que un founder
-- querría reglas.
...
-- finalize_vote: cuerpo grande preservado idéntico excepto el final
-- record_system_event para 'decision.finalized' que ahora captura
-- uuid_id + invoca el evaluator.
