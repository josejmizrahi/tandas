-- PARTE 12 — N.6 Disputes lifecycle + atom guards smoke.
--
-- Cobertura:
--   - open_dispute → group_disputes row + dispute.opened event.
--   - append_dispute_event → group_dispute_events row + dispute.event_added event (PARTE 3).
--   - assign_mediator → mediator_membership_id mutated.
--   - escalate_dispute_to_vote → status='escalated' + escalated_decision_id set + dispute.escalated event.
--   - record_dispute_resolution en otra dispute → status='resolved' + resolved_at + dispute.resolved event.
--   - group_dispute_events atom guards: UPDATE + DELETE blocked.
...
BEGIN
  ...
  -- 0a. Atom guards en group_dispute_events.
  ...
  -- N.6.1: open_dispute → row + dispute.opened.
  PERFORM set_config('request.jwt.claims', jsonb_build_object('sub', v_user_a::text)::text, true);
  v_dispute_id := public.open_dispute(
    v_group_id, 'general', NULL, 'Smoke dispute 1', 'desc', v_membership_b
  );
  ...
  -- N.6.2: append_dispute_event → row + dispute.event_added (PARTE 3).
  ...
  -- N.6.3: assign_mediator → mediator_membership_id mutado.
  ...
  -- N.6.4: escalate_dispute_to_vote → status='escalated' + escalated_decision_id + event.
  ...
  -- N.6.5: nueva dispute → record_dispute_resolution → status='resolved' + resolved_at.
  v_dispute_2_id := public.open_dispute(
    v_group_id, 'general', NULL, 'Smoke dispute 2', 'desc', v_membership_b
  );
  PERFORM public.record_dispute_resolution(
    v_dispute_2_id, 'mediated', 'resolved by mediation', jsonb_build_object('agreement','ok')
  );
  ...
  -- N.6.6: UPDATE group_dispute_events bloqueado (atom_no_mutation_guard).
  BEGIN
  ...
  -- N.6.7: DELETE group_dispute_events bloqueado.
  BEGIN
