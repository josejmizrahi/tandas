-- V3-D.19 — wire the resource branch in execute_decision.
-- Reads metadata.action; routes to canonical Resources RPCs that already
-- exist + are SECURITY DEFINER + do their own resources.* perm check.
--
-- Supported actions (4):
--   archive          → archive_resource(reference_id, 'decision_executed')
--   transfer         → set_resource_ownership(reference_id, 'individual', target, meta)
--                        requires metadata.target_membership_id
--   value_update     → update_resource_value(reference_id, amount, unit, 'decision')
--                        requires metadata.amount + metadata.unit
--   lifecycle_event  → record_resource_lifecycle_event(reference_id, event_type, payload, NULL)
--                        requires metadata.event_type
--
-- Doctrina: el caller de execute_decision DEBE tener tanto decisions.execute
-- como el resources.* correspondiente. Si falla la sub-RPC, la excepción
-- propaga y la decisión queda en 'passed' (no se marca executed). El branch
-- de auto-execution en finalize_vote ya hace swallow para no romper finalize.
...
BEGIN
  ...
  ELSIF v_d.reference_kind = 'resource' AND v_d.reference_id IS NOT NULL THEN
    -- V3-D.19 — wired. Routes by metadata.action. Inner RPCs do their
    -- own resources.* permission check; if the executor lacks it the
    -- exception propagates and the decision stays 'passed'.
    ...
    ELSE
      -- No action set OR unknown action — preserve D.18 behavior:
      -- execution succeeds in marking the decision done but no resource
      -- mutation. UI provenance carries the explanation.
      v_effects := v_effects || jsonb_build_object(
        'resource_action','not_implemented',
        'message','metadata.action ausente o desconocido (archive/transfer/value_update/lifecycle_event)'
      );
    END IF;
...
COMMENT ON FUNCTION public.execute_decision(uuid) IS
  'V3-D.19 — resource branch wired: archive/transfer/value_update/lifecycle_event routed to canonical Resources RPCs. Inner perms still enforced.';
