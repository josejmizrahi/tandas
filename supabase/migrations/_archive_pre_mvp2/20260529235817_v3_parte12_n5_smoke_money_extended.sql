...
BEGIN
  ...
  -- 0a. Hot-fix precheck: function source apunta a status='active' (no 'granted').
  ...
  -- N.5.5a: record_expense crea obligations → reverse_transaction RAISE.
  PERFORM set_config('request.jwt.claims', jsonb_build_object('sub', v_user_a::text)::text, true);
  ...
  -- N.5.6: custom split breakdown — amounts custom (no even).
  ...
  -- N.5.9: emit_mandate_expiring_events idempotent.
