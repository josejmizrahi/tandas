...
--   5. 'expelled' como membership state (spec drift).
--
...
DECLARE
  ...
  v_first text;
BEGIN
  -- N.14.1: 'granted' status no debe aparecer en prosrc (fue removed in hot-fix).
  ...
  -- N.14.2: 'decisions.propose' permission key no debe aparecer (fue hot-fix).
  SELECT count(*), min(p.proname)
    INTO v_n, v_first
    FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.prosecdef
  ...
  -- N.14.3: 'sanction.review' permission key no debe aparecer (fue hot-fix).
  SELECT count(*), min(p.proname)
    INTO v_n, v_first
    FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.prosecdef
  ...
  -- N.14.4: 'free_form' decision_type no debe aparecer (spec drift).
  SELECT count(*), min(p.proname)
    INTO v_n, v_first
    FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.prosecdef
  ...
  -- N.14.5: 'expelled' membership state no debe aparecer (canonical = 'banned').
  SELECT count(*), min(p.proname)
    INTO v_n, v_first
    FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.prosecdef
     AND p.prosrc ~ '''expelled''';
  step := 'N.14.5.no_dead_membership_state_expelled'; ok := v_n = 0;
  detail := 'count=' || v_n || COALESCE(' first=' || v_first, ''); RETURN NEXT;
