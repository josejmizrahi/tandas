-- D.22 hotfix: the smoke originally inserted into group_rules.body which
-- doesn't exist. Use create_text_rule RPC (canonical) which inserts into
-- both group_rules + group_rule_versions correctly.
-- NOTE: superseded by 20260601003210_d22_smoke_global_search_fix_rule_type
-- which corrects the 'text' rule_type (not in CHECK list) to 'norm'.
...
BEGIN
  ...
  -- Rule via canonical RPC (writes both group_rules + group_rule_versions)
  SELECT rule_id, version_id INTO v_rule_row
  FROM public.create_text_rule(v_group_a_id, 'Quasar quiet hours', 'No use after 10pm', 'text', 1);
