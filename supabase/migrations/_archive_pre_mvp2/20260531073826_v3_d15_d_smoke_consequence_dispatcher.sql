-- V3 FASE D.15 — Mig D: smoke R1-R5
--   R1 — resource.damaged -> create_obligation(counterparty=owner)
--   R2 — resource.archived -> start_vote                      (regression check)
--   R3 — resource.assigned -> send_notification(custodian)     (regression check)
--   R4 — invalid trigger/consequence combination -> rejected
--   R5 — lineage returns entity created
...
BEGIN
  ...
  -- Asset owned by B, custodian initially A
  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  v_asset := public.create_group_resource(
    v_group_id, 'asset', 'Smoke D15 Asset', 'B owns, A is custodian',
    'members', 'individual', v_membership_b, v_membership_a);
  step := 'D15.setup.asset_created'; ok := v_asset.id IS NOT NULL
    AND v_asset.owner_membership_id = v_membership_b;
  detail := 'asset_id=' || COALESCE(v_asset.id::text,'NULL')
            || ' owner=B'; RETURN NEXT;

  -- =========================================================================
  -- R1 — resource.damaged -> create_obligation(counterparty=owner)
  --      Actor=A, owner=B -> obligation A owes B 100 MXN.
  -- =========================================================================
  ...
  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  PERFORM public.mark_asset_condition(v_asset.id, 'damaged', 'smoke D15 R1', 'smoke-d15-r1');
  ...
  -- =========================================================================
  -- R2 — resource.archived -> start_vote (regression: D.14 path still works)
  -- =========================================================================
  ...
  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  PERFORM public.archive_resource(v_asset.id, 'smoke D15 R2 archive');
  ...
  -- =========================================================================
  -- R3 — resource.assigned -> send_notification(custodian)  (audience custodian
  --      resolved to NEW custodian after reassign; with recipient_user_ids[])
  --
  --      Cannot reassign on an archived resource; create a fresh asset for R3.
  -- =========================================================================
  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  v_asset := public.create_group_resource(
  ...
  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  PERFORM public.assign_asset_custodian(v_asset.id, v_membership_b, 'reassign D15', 'smoke-d15-r3');
  SELECT count(*) INTO v_matched_count FROM public.group_rule_evaluations
   WHERE rule_version_id = v_r3_version_id AND matched = true;
  SELECT count(*) INTO v_outbox_after FROM public.notifications_outbox
   WHERE group_id = v_group_id AND category = 'rule_consequence' AND recipient_user_id = v_user_b;

  -- Verify actions_emitted captured recipient_user_ids[]
  ...
  -- =========================================================================
  -- R4 — invalid combo rejected. trigger.mandate.granted only allows
  --      consequence.send_notification, so issue_sanction must be rejected.
  -- =========================================================================
  v_rejected := false; v_rejected_msg := NULL;
  BEGIN
    PERFORM public.create_engine_rule(
      v_group_id,
      'D15 R4 — should be rejected',
      'trigger.mandate.granted',
      NULL,
      jsonb_build_array(jsonb_build_object(
  ...
  detail := 'rejected=' || v_rejected::text
            || ' msg_excerpt=' || COALESCE(left(v_rejected_msg, 120),'NULL');
  RETURN NEXT;

  -- =========================================================================
  -- R5 — lineage returns the obligation entity created in R1.
  -- =========================================================================
  ...
  detail := 'lineage_rows=' || v_lineage_count
            ...
            || ' label=' || COALESCE(v_lineage_sample->>'target_label','NULL');
  RETURN NEXT;
