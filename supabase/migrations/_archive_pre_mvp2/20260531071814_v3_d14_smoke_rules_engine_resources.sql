-- V3 FASE D.14 — Mig C: smoke for resource-aware rule engine (R1..R5)
-- Substitutions from founder's prompt (per approved scope):
--   R2: create_obligation -> create_pool_charge (no peer-to-peer obligation in D.14)
--   R5: require_approval  -> start_vote(decision_type='proposal')
-- Resets ruul.rule_eval_depth between event-emitting RPCs because all 7 events
-- fire in ONE transaction and the GUC accumulates across nested triggers.
...
DECLARE
  ...
  v_used_event_uuid uuid;
  v_damaged_event_uuid uuid;
  v_value_low_event_uuid uuid;
  v_value_high_event_uuid uuid;
  v_assigned_event_uuid uuid;
  v_archived_event_uuid uuid;
BEGIN
  ...
  -- Create an asset (custodian = A, group-owned)
  ...
  -- =========================================================================
  -- R1: WHEN resource.used THEN send_notification(audience='actor')
  -- =========================================================================
  ...
  PERFORM public.record_resource_lifecycle_event(
    ...
    jsonb_build_object('membership_id', v_membership_a::text),
    'smoke-d14-r1');
  ...
  -- =========================================================================
  -- R2: WHEN resource.damaged THEN create_pool_charge (50 MXN, kind='fee')
  -- =========================================================================
    ...
    FROM public.create_engine_rule(
      ...
      jsonb_build_array(jsonb_build_object(
        ...
        'fields', jsonb_build_object(
          ...
          'currency', 'MXN',
          'charge_kind', 'fee',
          'reason', 'Daño recurso D14 R2'))),
      'norm', 1) cer;

  SELECT count(*) INTO v_obligation_count
    FROM public.group_obligations
   WHERE group_id = v_group_id AND debtor_membership_id = v_membership_a;

  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  PERFORM public.mark_asset_condition(v_asset.id, 'damaged', 'smoke D14 R2 damaged', 'smoke-d14-r2');

  SELECT count(*) - v_obligation_count INTO v_obligation_count
    FROM public.group_obligations
   WHERE group_id = v_group_id AND debtor_membership_id = v_membership_a;
  SELECT count(*) INTO v_matched_count FROM public.group_rule_evaluations
  ...
  detail := 'matched=' || v_matched_count
            || ' obligations_delta=' || v_obligation_count; RETURN NEXT;

  -- =========================================================================
  -- R3: WHEN resource.value_updated
  --      IF condition.resource_compare(atom=resource.value, op='>', value='10000')
  --      THEN send_notification(audience='group')
  -- =========================================================================
  SELECT cer.rule_id, cer.version_id INTO v_r3_rule_id, v_r3_version_id
    FROM public.create_engine_rule(
      ...
      jsonb_build_object(
        'kind','condition.resource_compare',
        'fields', jsonb_build_object(
          'atom','resource.value',
          'op','>',
          'value','10000')),
  ...
  -- (b) high update -> should match + notify group (2 recipients: A and B)
  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  ...
  ok := v_unmatched_count >= 1 AND v_matched_count >= 1
        AND (v_outbox_after - v_outbox_before) >= 2;
  detail := 'unmatched_low=' || v_unmatched_count
            || ' matched_high=' || v_matched_count
            || ' outbox_delta=' || (v_outbox_after - v_outbox_before); RETURN NEXT;

  -- =========================================================================
  -- R4: WHEN resource.assigned THEN send_notification(audience='custodian')
  --      Tests NEW custodian audience resolver. After reassign to B, the
  --      custodian-at-eval-time IS B -> B receives notification.
  -- =========================================================================
  SELECT cer.rule_id, cer.version_id INTO v_r4_rule_id, v_r4_version_id
    FROM public.create_engine_rule(
      v_group_id, 'D14 R4 — notify custodian on assign',
      'trigger.resource.assigned',
      NULL,
      jsonb_build_array(jsonb_build_object(
        'kind','consequence.send_notification',
  ...
  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  PERFORM public.assign_asset_custodian(v_asset.id, v_membership_b, 'reasignación R4', 'smoke-d14-r4');
  ...
  detail := 'matched=' || v_matched_count
            || ' outbox_to_B_delta=' || (v_outbox_after - v_outbox_before); RETURN NEXT;

  -- =========================================================================
  -- R5: WHEN resource.archived THEN start_vote(decision_type='proposal')
  --      (substitute for require_approval per founder scope)
  -- =========================================================================
  SELECT cer.rule_id, cer.version_id INTO v_r5_rule_id, v_r5_version_id
    FROM public.create_engine_rule(
      v_group_id, 'D14 R5 — vote on archive',
      'trigger.resource.archived',
      NULL,
      jsonb_build_array(jsonb_build_object(
  ...
  SELECT count(*) INTO v_decision_count
   ...
   WHERE group_id = v_group_id
     AND metadata->>'engine_rule_version_id' = v_r5_version_id::text;

  PERFORM set_config('ruul.rule_eval_depth', '0', true);
  PERFORM public.archive_resource(v_asset.id, 'smoke D14 R5 archive');

  SELECT count(*) - v_decision_count INTO v_decision_count
    FROM public.group_decisions
   WHERE group_id = v_group_id
     AND metadata->>'engine_rule_version_id' = v_r5_version_id::text;
  SELECT count(*) INTO v_matched_count FROM public.group_rule_evaluations
  ...
  detail := 'matched=' || v_matched_count
            || ' decisions_delta=' || v_decision_count; RETURN NEXT;

  -- Cleanup note (append-only tables block cascade delete; smoke data persists on dev)
