-- D.21B fix3 — mark outbox rows dispatched before delete (trigger blocks pending deletes)
...
begin
  ...
  insert into public.notifications_outbox (recipient_user_id, category, payload, dispatch_status, dispatched_at)
    values (v_uid, 'test.inbox', '{"message":"first"}'::jsonb, 'dispatched', now()) returning id into v_id1;
  insert into public.notifications_outbox (recipient_user_id, category, payload, dispatch_status, dispatched_at)
    values (v_uid, 'test.inbox', '{"message":"second"}'::jsonb, 'dispatched', now()) returning id into v_id2;
  insert into public.notifications_outbox (recipient_user_id, category, payload, dispatch_status, dispatched_at)
    values (v_other, 'test.inbox', '{"message":"foreign"}'::jsonb, 'dispatched', now()) returning id into v_id3;
