...
COMMENT ON FUNCTION public.group_decision_rules(uuid) IS
  'Primitivas 6/16/22 Foundation (mig 20260527060000): returns the active decision rules for a group (default_style/quorum_min/notes). Defaults are baked in when the jsonb is empty. Active-member gate.';
...
COMMENT ON FUNCTION public.set_decision_rules(uuid, text, int, text) IS
  'Primitivas 6/16/22 Foundation (mig 20260527060000): upsert groups.decision_rules in-place. Validates style and quorum_min. Requires permission group.update.';
