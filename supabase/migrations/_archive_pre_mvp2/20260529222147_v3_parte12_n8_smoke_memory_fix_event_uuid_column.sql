-- PARTE 12 — N.8 fix: group_events_recent retorna `event_uuid` (no `id` bigint).
-- Cursor exposing real = occurred_at DESC. Spec §N.8.3 decía "id DESC" pero
-- el RPC no expone el bigint cursor — drift documentado.
...
BEGIN
  ...
  -- pg_sleep para garantizar occurred_at distinto entre los 2 eventos.
  ...
  -- N.8.4: group_events_recent retorna composite (event_uuid + occurred_at).
  -- El primer row debe ser el evento más reciente (v_event_uuid_2).
  ...
  SELECT event_uuid INTO v_second_uuid FROM public.group_events_recent(v_group_id, 50, NULL) OFFSET 1 LIMIT 1;
  v_ordering_ok := v_first_uuid = v_event_uuid_2 AND v_second_uuid IS NOT NULL;
