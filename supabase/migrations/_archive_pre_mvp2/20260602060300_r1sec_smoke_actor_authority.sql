-- R.1-SEC Smoke — _smoke_r1sec_actor_authority()
--
-- 10 casos requeridos por el plan R.1:
--   1. Person actor puede ver su propio net worth.
--   2. Person actor NO puede ver net worth de otro actor.
--   3. Miembro activo puede ver group_world_summary.
--   4. No-miembro NO puede ver group_world_summary.
--   5. Usuario sin autoridad NO puede grant OWN sobre resource ajeno.
--   6. Owner/MANAGE sí puede grant VIEW.
--   7. Usuario sin autoridad NO puede create shareholder_of falso.
--   8. Usuario autorizado sí puede crear relación.
--   9. anon no puede ejecutar summaries sensibles.
--  10. RLS critical tables enabled.
...
BEGIN
  ...
  -- Resource personal de user_a + OWN right (setup directo, bypass RPCs)
  ...
  VALUES (v_resource, v_user_a, 'OWN', 100, '{"source": "_smoke_r1sec_setup"}'::jsonb);
  ...
  -- (user_b intenta otorgarse OWN sobre el resource personal de user_a)
  v_caught := false;
  BEGIN
  ...
  -- (user_b intenta declarar que user_a es shareholder de v_other)
  PERFORM set_config('request.jwt.claims', jsonb_build_object('sub', v_user_b::text)::text, true);
  v_caught := false;
  BEGIN
  ...
  -- (user_a se declara a sí mismo shareholder_of v_other — subject = self)
  PERFORM set_config('request.jwt.claims', jsonb_build_object('sub', v_user_a::text)::text, true);
  v_rel_id := public.create_actor_relationship(
    p_subject_actor_id := v_user_a,
    p_relationship_type := 'shareholder_of',
...
COMMENT ON FUNCTION public._smoke_r1sec_actor_authority() IS
  'Smoke R.1-SEC: 10 casos de autorización actor-céntrica (authority, gating de summaries, hardening de grant/revoke/relationships, grants anon, RLS).';
