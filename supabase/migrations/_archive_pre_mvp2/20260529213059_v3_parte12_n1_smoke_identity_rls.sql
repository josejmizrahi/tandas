-- PARTE 12 — N.1 Identity + RLS smoke
-- Cierra los 4 escenarios del spec §N.1 + 1 precheck que PARTE 1 policy esté viva.
-- Pattern: SECURITY DEFINER + SET LOCAL ROLE (authenticated|anon) para forzar evaluación de RLS.
...
BEGIN
  ...
  v_invite_c_id := public.invite_member(v_group_ac, 'smoke-rls-c@test', NULL, 'member', NULL);
