-- d24_p5_decision_execution_observability
...
-- =====================================================================
-- V3 D.24 PHASE 5 — Decision Execution Observability
--
-- Scope (founder-expanded):
--   1. Add 6 cols to group_decisions:
--        execution_status, execution_error, execution_attempts,
--        execution_payload, execution_started_at, execution_finished_at
--   2. Estados: not_required / pending / executing / executed /
--               failed / blocked / cancelled
--   3. Backfill from current group_decisions state.
--   4. Refactor execute_decision: idempotent + reentrant-safe via row
--      lock + crash-safe via sub-block EXCEPTION handler. Emit lifecycle
--      events: decision.execution_started / .execution_completed /
--      .execution_failed / .execution_blocked.
--   5. NO job queue, NO automatic retries (founder scope).
--
-- Contract iOS preserve: ExecuteDecisionResult.status is free-form text;
-- 'failed' is a NEW valid value (in addition to 'executed' / 'rejected' /
-- 'cancelled'). iOS today reads `status` raw — handler can branch on it.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. Add columns (NULL-safe first; CHECK + DEFAULT after backfill)
-- ---------------------------------------------------------------------
...
set execution_status = case
        ...
        when status = 'passed'       then 'pending'
        else 'not_required'
    end,
    ...
    execution_attempts = case
        when executed_at is not null then 1
...
alter table public.group_decisions
    alter column execution_status set default 'not_required',
    alter column execution_status set not null;

alter table public.group_decisions
    drop constraint if exists group_decisions_execution_status_check;
alter table public.group_decisions
    add constraint group_decisions_execution_status_check
    check (execution_status in (
        'not_required','pending','executing','executed','failed','blocked','cancelled'
    ));

-- Index for retry sweep (later phases). 0-effect on read-time today.
create index if not exists group_decisions_execution_status_idx
    on public.group_decisions(execution_status)
    where execution_status in ('pending','executing','failed','blocked');

-- ---------------------------------------------------------------------
-- 4. Refactor execute_decision — observability + idempotent
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  -- Row-level lock holds across the whole function. Concurrent
  -- callers wait here, then re-check execution_status below.
  ...
  -- Idempotency: already executed → return cached payload (no re-emit).
  IF v_d.execution_status = 'executed' THEN
    decision_id := v_d.id;
    status      := 'executed';
    outcome     := (v_d.result->>'outcome');
    effects     := COALESCE(v_d.execution_payload, '{}'::jsonb)
                || jsonb_build_object('reason','already_executed');
    RETURN NEXT;
    RETURN;
  END IF;

  -- Other terminal decision statuses short-circuit (legacy contract).
  ...
  -- Mark executing. This UPDATE persists if the function returns
  -- (success or caught failure). If outer raises, rolls back.
  UPDATE public.group_decisions
  ...
  -- ===================== Dispatch (preserved from D.18-F.1) =====================
  BEGIN
    ...
    ELSIF v_d.reference_kind = 'resource' AND v_d.reference_id IS NOT NULL THEN
      ...
      ELSIF v_res_action = 'value_update' THEN
        ...
        v_effects := v_effects || jsonb_build_object('resource_value_updated', v_d.reference_id,
    ...
    ELSIF v_d.reference_kind = 'group' AND v_action_key IS NOT NULL THEN
      ...
      ELSIF v_action_key = 'group.visibility.set' THEN
        ...
        IF v_grp_visibility IS NULL THEN
          RAISE EXCEPTION 'group.visibility.set requires metadata.visibility' USING errcode = '22023';
        END IF;
      ...
      ELSIF v_action_key = 'group.boundary.set' THEN
        PERFORM public.set_group_boundary_policy(
          v_d.group_id,
          NULLIF(v_d.metadata->>'entry_mode',''),
          NULLIF(v_d.metadata->>'who_can_invite',''),
          COALESCE((v_d.metadata->>'requires_approval')::boolean, true),
          NULLIF(v_d.metadata->>'exit_mode',''),
      ...
      ELSIF v_action_key = 'group.decision_rules.set' THEN
        PERFORM public.set_decision_rules(
          v_d.group_id,
    ...
    ELSIF v_d.reference_kind = 'role' AND v_action_key IS NOT NULL THEN
      IF v_action_key = 'role.create' THEN
        ...
        v_role_id := public.create_custom_role(
          v_d.group_id,
          NULLIF(v_d.metadata->>'key',''),
          NULLIF(v_d.metadata->>'name',''),
          NULLIF(v_d.metadata->>'description',''),
          v_perm_keys);
    ...
    ELSIF v_d.reference_kind = 'money_movement' AND v_action_key IS NOT NULL THEN
      IF v_action_key = 'money.payout' THEN
        ...
        IF v_pay_amount IS NULL OR v_pay_amount <= 0 OR v_pay_to_mid IS NULL THEN
          RAISE EXCEPTION 'money.payout requires metadata.amount + to_membership_id' USING errcode = '22023';
        END IF;
        v_payout_id := public.record_payout(
          v_d.group_id, v_pay_to_mid, v_pay_amount, v_pay_unit,
          NULLIF(v_d.metadata->>'source_resource_id','')::uuid,
          v_pay_reason,
          NULLIF(v_d.metadata->>'mandate_id','')::uuid,
      ...
      ELSIF v_action_key = 'money.transaction.reverse' THEN
        ...
        IF v_reverse_txn_id IS NULL THEN
          RAISE EXCEPTION 'money.transaction.reverse requires transaction_id' USING errcode = '22023';
        END IF;
    ...
    ELSIF v_d.reference_kind = 'norm' AND v_action_key = 'norm.promote_to_rule' AND v_d.reference_id IS NOT NULL THEN
      ...
      SELECT rule_id INTO v_promoted_rule
        FROM public.promote_norm_to_rule(v_norm_id, v_norm_rule_type, v_norm_severity);
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Sub-block savepoint rolls back side effects + any record_system_event
    -- inserts done within the block. Outer transaction is still alive →
    -- the UPDATEs below persist.
    v_err_msg   := SQLERRM;
    v_err_state := SQLSTATE;

    UPDATE public.group_decisions
       SET execution_status      = 'failed',
           execution_error       = v_err_msg,
           execution_finished_at = now(),
           execution_payload     = jsonb_build_object('error', v_err_msg, 'sqlstate', v_err_state)
     WHERE id = p_decision_id;

    PERFORM public.record_system_event(
      v_d.group_id, 'decision.execution_failed', 'decision', p_decision_id,
  ...
  -- Success path. Persist executed state + payload + finished_at.
  UPDATE public.group_decisions
     SET status                = 'executed',
         executed_at           = now(),
         executed_by           = v_uid,
         ...
         execution_finished_at = now(),
         execution_payload     = v_effects,
         execution_error       = NULL
   WHERE id = p_decision_id;

  -- Keep legacy `decision.executed` event for backwards-compat with
  -- existing consumers (rule engine, inbox dispatcher, etc).
  SELECT rse.uuid_id INTO v_event_uuid FROM public.record_system_event(
    ...
    jsonb_build_object(
      ...
      'template_key',   v_d.template_key,
      'action_key',     v_action_key,
      'effects',        v_effects)) rse;
  PERFORM public.evaluate_rules_for_event(v_event_uuid, 'sync');

  -- D.24 PHASE 5: terminal completion event.
  PERFORM public.record_system_event(
    v_d.group_id, 'decision.execution_completed', 'decision', p_decision_id,
    'Ejecución completada',
    jsonb_build_object(
      'reference_kind', v_d.reference_kind,
      'reference_id',   v_d.reference_id,
      'action_key',     v_action_key,
      'attempts',       v_d.execution_attempts + 1,
      'effects',        v_effects));
