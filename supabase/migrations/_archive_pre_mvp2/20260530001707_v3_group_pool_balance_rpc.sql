...
BEGIN
  ...
  -- Contributions to pool (to_membership_id IS NULL)
  ...
  -- Settlement payments to pool (pago de multas)
  ...
  -- Payouts from pool (cuando el grupo paga algo)
  ...
  -- Reversals que afectan al pool (revierten un movimiento previo
  -- contra el pool). Signo: si revierte una contribution salida,
  -- decrece el pool; si revierte un payout, lo aumenta. Lo más simple:
  -- sumar signed via metadata.direction si existiera; mientras no,
  -- los reversals quedan como informativos.
  SELECT COALESCE(SUM(amount * CASE
    WHEN to_membership_id IS NULL THEN -1   -- revierte algo que entró al pool
    WHEN from_membership_id IS NULL THEN 1  -- revierte algo que salió del pool
    ELSE 0
