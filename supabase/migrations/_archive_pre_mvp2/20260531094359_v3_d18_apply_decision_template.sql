-- V3-D.18 follow-up — apply_decision_template(decision_id, template_key)
-- iOS llama este RPC inmediatamente después de start_vote cuando el usuario
-- eligió una plantilla en ProposeDecisionSheet. Persiste template_key +
-- execution_mode + metadata del template en la decisión recién creada.
-- Solo permitido mientras status='open' (no retro-apply).
-- Gate: caller debe tener decisions.create en el grupo (la misma perm que
-- start_vote ya verifica).
...
BEGIN
  ...
  SELECT * INTO v_t FROM public.decision_templates_catalog WHERE template_key = p_template_key;
...
GRANT EXECUTE ON FUNCTION public.apply_decision_template(uuid, text) TO authenticated;

COMMENT ON FUNCTION public.apply_decision_template(uuid, text) IS
  'V3-D.18 — stamp template_key + execution_mode + merge metadata on a freshly-created decision. Only while status=open.';
