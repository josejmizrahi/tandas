-- PARTE 12 — N.7 fix3: cast_vote tiene 2 overloads (4-arg y 5-arg con p_weight).
-- PG no puede resolver con NULL untyped. Uso named args para disambiguar al 4-arg.
...
BEGIN
  ...
  -- Named args disambigua al 4-arg overload (no p_weight).
  v_vote_1_id := public.cast_vote(p_decision_id := v_decision_id, p_option_id := NULL, p_vote_value := 'yes', p_reason := 'first');
  v_vote_2_id := public.cast_vote(p_decision_id := v_decision_id, p_option_id := NULL, p_vote_value := 'no',  p_reason := 'changed_mind');
