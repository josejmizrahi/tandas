-- D.24.4 hotfix: the original RPC carries `#variable_conflict use_column`
-- so the output column names (membership_id, group_id, status) don't
-- shadow same-named columns inside the function body. Dropping it broke
-- the inner `SELECT count(*) FROM group_member_roles WHERE
-- membership_id = p_membership_id`. Restore the directive.
...
BEGIN
  ...
  -- D.24.4: surface the approval to the new member's Inbox + push.
  -- Skip silently if the requester is a placeholder (no auth user yet).
