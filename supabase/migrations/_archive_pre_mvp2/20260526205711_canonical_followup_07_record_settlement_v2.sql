-- record_settlement v2:
--   * acepta p_mandate_id + valida scope
--   * persiste mandate_id en settlements y en la transaction resultante
--   * authority_path en group_events.payload
--   * arregla idempotency bug (early return ahora respeta `returns table`)
--   * agrega sync rule evaluation
--
-- Drop old signature first.

drop function if exists public.record_settlement(uuid, uuid, uuid, text, numeric, text, text, text);
...
begin
  ...
  -- Idempotency: si client_id ya existe, devuelve par previo y return.
  if p_client_id is not null then
    select id, ledger_entry_id into v_settlement, v_existing_tx
