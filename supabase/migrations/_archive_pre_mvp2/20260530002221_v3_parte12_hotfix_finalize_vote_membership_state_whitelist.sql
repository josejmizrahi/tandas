...
-- o 'inactive' pasaba la validación local pero raise dentro de set_membership_state
-- → finalize_vote rollback completo, decisión queda stuck en 'passed' nunca
-- ejecutado, o aborta antes de marcar 'passed'. Votar expulsión = roto silently.
--
-- Hot-fix mechanical: alinear IN list a la whitelist real. Mapping:
--   - 'expelled' → 'banned' (canonical kick-out).
--   - 'inactive' → 'left' (más cercano semántico).
--
-- Mantengo el resto de finalize_vote idéntico — solo cambio el IN list y la
-- traducción de target_state antes de invocar set_membership_state.
