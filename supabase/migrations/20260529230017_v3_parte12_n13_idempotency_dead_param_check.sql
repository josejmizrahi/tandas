-- PARTE 12 — N.13.3: extender meta-smoke con dead-param check de p_client_id.
--
-- Detecta RPCs SECURITY DEFINER que declaran `p_client_id text` pero NUNCA lo
-- referencian en el body. Esta sesión cazó 4:
--   - record_payout (storeaba en column pero sin memo lookup — UNIQUE raise).
--   - record_pool_charge (storeaba en metadata jsonb, sin UNIQUE → double-charge).
--   - submit_rsvp (param 100% muerto).
--   - submit_check_in (param 100% muerto).
--
-- Los 2 primeros ya tienen hot-fix (migs #5+#6 PARTE 12). Los 2 últimos son
-- DEFERRED pending founder doctrine sobre dónde almacenar client_id —
-- whitelistados temporalmente.
--
-- Bug del regex anterior: PG usa `\y` para word boundary (no `\b`).
