...
comment on table public.governance_policies is
  'R.5.1: politicas de gobernanza por contexto (key -> value jsonb).';
...
comment on table public.governance_actions is
  'R.5.5/5.6: accion critica y la decision que la gobierna (auditoria).';
...
declare
  ...
  v_decision uuid;
  v_ga uuid;
  v_title text := coalesce(nullif(btrim(coalesce(p_title, '')), ''), 'Accion de gobernanza: ' || p_action_key);
begin
  if v_caller is null then raise exception 'unauthenticated' using errcode = '28000'; end if;
...
-- 5.4 consent + quorum + threshold en close_decision (backward-compatible)
...
-- 5.6 mandatory governance gate en remove_member (opt-in)
...
values
  ('governance.policy_set',        'governance', 'Se definio una politica de gobernanza', 'governance_policy', false),
  ('governance.policy_removed',    'governance', 'Se elimino una politica de gobernanza', 'governance_policy', false),
  ('governance.vote_delegated',    'governance', 'Un actor delego su voto',               'vote_delegation',   false),
  ('governance.delegation_revoked','governance', 'Un actor revoco su delegacion de voto', 'vote_delegation',   false),
  ('governance.action_requested',  'governance', 'Se propuso una accion de gobernanza',   'governance_action', false),
  ('governance.action_executed',   'governance', 'Se ejecuto una accion de gobernanza',   'governance_action', false)
