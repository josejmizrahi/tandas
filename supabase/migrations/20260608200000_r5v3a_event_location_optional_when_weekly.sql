-- R.5V.3A.event (2026-06-08) — Extiende F.EVENT.5: location es opcional
-- cuando el evento es recurring weekly (host rota cada semana, doctrina
-- F.EVENT.8). El anfitrión actual define la ubicación de su ocurrencia
-- vía edit_event sin contaminar las siguientes.
--
-- Reemplaza ambos write-paths (create + update). Resto de la lógica
-- queda idéntica al estado anterior.
...
begin
  ...
  v_rule_norm := lower(btrim(coalesce(p_recurrence_rule, '')));

  -- F.EVENT.5 + R.5V.3A.event: location es opcional cuando el evento es
  -- recurring weekly (host rota — cada anfitrión decide su semana).
  if not p_is_virtual and v_location is null
     and v_rule_norm <> 'weekly'
     and v_rule_norm not like '%freq=weekly%' then
    raise exception 'location_required: events must have a location unless marked as virtual or recurring weekly'
      using errcode = '22023';
  end if;

  -- F.EVENT.9: bounds requieren recurrence_rule.
...
begin
  ...
  v_rule_norm := lower(coalesce(v_new_recurrence, ''));

  -- F.EVENT.5 + R.5V.3A.event: location es opcional cuando el evento es
  -- recurring weekly (host rota — cada anfitrión decide su semana).
  if not v_new_is_virtual
     and (v_new_location is null or length(btrim(v_new_location)) = 0)
     and v_rule_norm <> 'weekly'
     and v_rule_norm not like '%freq=weekly%' then
    raise exception 'location_required: events must have a location unless marked as virtual or recurring weekly'
      using errcode = '22023';
