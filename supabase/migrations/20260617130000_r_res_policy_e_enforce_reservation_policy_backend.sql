-- R.RES.POLICY.E — backend enforcement de reservation_policy
--
-- iOS valida (R.RES.POLICY.A+B) pero sin enforcement server-side un caller
-- malicioso o legacy puede bypass min/max + advance_window. Doctrina founder
-- "completos y conectados": iOS guía + backend enforza.
--
-- Lookup chain:
--   resource → resources.resource_subtype_key
--           → resource_subtypes.metadata->reservation_policy
-- Si NO hay subtype_key o NO hay policy, skip (back-compat con resources
-- legacy creados antes del Subtype Picker).
--
-- Checks agregados (todos errcode '22023' invalid_parameter_value):
--   1. granularity='none' → recurso NO reservable.
--   2. duración < min_duration_units → 'duration_below_minimum'.
--   3. duración > max_duration_units (si != null) → 'duration_above_maximum'.
--   4. starts_at > now() + advance_window_days → 'advance_window_exceeded'.
...
begin
  ...
  select r.resource_subtype_key into v_subtype_key
    from public.resources r
   where r.id = p_resource_id;

  if v_subtype_key is not null then
  ...
  if v_policy is not null then
    ...
    v_min_units := (v_policy->>'min_duration_units')::integer;
