-- 00370 — Splitwise-style split modes on expense writers.
--
-- Why
-- ===
-- Splitwise offers 4 main split modes: equal, exact (per-person $),
-- percentage, and shares (relative units). Our V1 implementation only
-- supported equal-split via the `metadata.participants` array
-- (mig 00367). This mig extends `fund_record_expense` +
-- `record_shared_expense` to accept the chosen mode + the canonical
-- per-member breakdown in cents.
--
-- Data model
-- ==========
-- The amount_cents column stays the canonical TOTAL of the expense.
-- The new metadata keys are informational:
--   * `metadata.split_mode`       text  ('equal' | 'exact' | 'percent' | 'shares')
--   * `metadata.split_breakdown`  jsonb array of {member_id, share_cents}
--     — every projection reads `amount_cents` for math; breakdown
--       drives per-member "owes you $X" UX rendering.
-- `metadata.participants` is still stamped (as the set of member_ids
-- with share > 0) for backward compat with the V1 readers that only
-- know about that key.
--
-- V1 posture: no auto-IOU generation. The breakdown is metadata-only;
-- users still settle manually via the existing settle-up flow. V2 can
-- emit one ledger atom per participant to make
-- `member_balances_per_group` reflect splits natively.
...
begin
  ...
  if p_split_mode is not null and p_split_mode not in ('equal','exact','percent','shares') then
    raise exception 'fund_record_expense: invalid split_mode %', p_split_mode using errcode = '22023';
  end if;
  ...
  -- mig 00370: persist mode + breakdown when supplied.
  if p_split_mode is not null then
    v_payload_meta := v_payload_meta || jsonb_build_object('split_mode', p_split_mode);
  end if;
  if p_split_breakdown is not null and jsonb_array_length(p_split_breakdown) > 0 then
    v_payload_meta := v_payload_meta || jsonb_build_object('split_breakdown', p_split_breakdown);
  end if;
...
revoke execute on function public.fund_record_expense(uuid, bigint, uuid, text, text, uuid, uuid, uuid, uuid, uuid[], text, jsonb) from public, anon;
grant  execute on function public.fund_record_expense(uuid, bigint, uuid, text, text, uuid, uuid, uuid, uuid, uuid[], text, jsonb) to authenticated;
comment on function public.fund_record_expense(uuid, bigint, uuid, text, text, uuid, uuid, uuid, uuid, uuid[], text, jsonb) is
  'v6 (mig 00370): Splitwise-style modes — accepts p_split_mode (equal/exact/percent/shares) + p_split_breakdown jsonb, stamps into metadata. V1 metadata-only (no auto IOU emission).';
...
begin
  ...
  v_entry := public.fund_record_expense(
    ...
    p_split_mode         => p_split_mode,
    p_split_breakdown    => p_split_breakdown
  );
...
revoke execute on function public.record_shared_expense(uuid, bigint, uuid, text, text, uuid, uuid, uuid, uuid[], text, jsonb) from public, anon;
grant  execute on function public.record_shared_expense(uuid, bigint, uuid, text, text, uuid, uuid, uuid, uuid[], text, jsonb) to authenticated;
comment on function public.record_shared_expense(uuid, bigint, uuid, text, text, uuid, uuid, uuid, uuid[], text, jsonb) is
  'v3 (mig 00370): wrapper passes p_split_mode + p_split_breakdown through to fund_record_expense.';
