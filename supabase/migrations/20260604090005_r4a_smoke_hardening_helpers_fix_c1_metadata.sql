...
begin
  ...
  -- C1: trust_edges updated_at trigger registered correctly
  -- (cannot timestamp-diff inside a single txn: touch_updated_at uses now(),
  --  which is transaction_timestamp() and stays constant)
  if not exists (
    select 1
      from pg_trigger t
  ...
  -- Sanity: the trigger fires (after UPDATE, updated_at must equal now()
  -- for the current transaction, even if it didn't advance).
  ...
  -- C2: actors.is_context defaults to false on fresh insert
  ...
  if (select is_context from public.actors where id = v_new_actor_id) <> false then
    raise exception 'r4a C2: is_context did not default to false';
  end if;

  -- C3: backfill set all 7 operational subtypes to is_context = true
  select count(*) into v_back_count
   ...
   where actor_subtype in ('friend_group','family','company','trust','trip','community','project')
     ...
     and id <> v_new_actor_id;

  if v_back_count <> 0 then
    raise exception 'r4a C3: % operational actors not backfilled', v_back_count;
  end if;
  ...
  delete from public.activity_events where context_actor_id = v_familia
     or actor_id in (v_person, v_other_person);
  ...
  raise notice '_smoke_r4a_hardening_helpers passed (10 casos)';
end;
$$;
