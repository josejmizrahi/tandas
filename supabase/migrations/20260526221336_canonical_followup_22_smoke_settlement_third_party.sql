-- Smoke ampliado: agrega step 7.p10_settlement_third_party_blocked
...
begin
  ...
  -- también invitamos a C para tener un baseline member que intente la fraud
  ...
  -- P7: user_c (baseline member) intenta registrar gasto a nombre de user_a sin mandate
  perform set_config('request.jwt.claims', jsonb_build_object('sub', v_user_c::text)::text, true);
  begin
  ...
  -- P10: user_c (baseline member) intenta registrar SETTLEMENT entre user_a y user_b
  -- (cerrar deuda de un tercero) sin mandate. Debe fallar.
  -- Pre-condicion: necesitamos una obligación abierta entre A y B. Creamos otro expense.
  perform set_config('request.jwt.claims', jsonb_build_object('sub', v_user_a::text)::text, true);
  perform public.record_expense(
