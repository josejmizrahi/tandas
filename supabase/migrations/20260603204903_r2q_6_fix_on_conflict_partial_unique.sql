-- Fix: ON CONFLICT ON CONSTRAINT no funciona con partial unique indexes (no son
-- constraints — son índices). Reescribimos las cláusulas con ON CONFLICT
-- (cols) WHERE … que PostgreSQL sí matchea contra el partial unique.
