-- D.22 FASE F.1 — Extend execute_decision to handle the 5 new reference_kinds
-- introduced by FASE D templates: group, role, money_movement, norm, plus
-- resource.unarchive and dissolution.finalize. Also fix mandate_grant
-- reference_kind drift.

-- Fix template reference_kind to match existing execute_decision branch.
UPDATE public.decision_templates_catalog
   SET reference_kind = 'mandate_grant'
 WHERE template_key = 'decision.mandate_grant';

-- Replace execute_decision with extended branches.
...
BEGIN
  ...
  ELSIF v_d.reference_kind = 'dissolution' AND v_d.reference_id IS NOT NULL THEN
    -- decision.dissolution_finalize → finalize; else approve.
  ...
  ELSIF v_d.reference_kind = 'rule' AND v_d.reference_id IS NOT NULL THEN
    v_rule_action := COALESCE(NULLIF(v_d.metadata->>'action', ''),
                              NULLIF(v_d.metadata->>'rule_action',''));
  ...
  ELSIF v_d.reference_kind = 'resource' AND v_d.reference_id IS NOT NULL THEN
    v_res_action := COALESCE(NULLIF(v_d.metadata->>'action', ''),
                             NULLIF(v_d.metadata->>'resource_action',''));
  ...
  -- ======================= NEW BRANCHES (FASE F.1) =======================

  ELSIF v_d.reference_kind = 'group' AND v_action_key IS NOT NULL THEN
    -- Routes by metadata.action_key for engine.toggle / visibility / boundary / decision_rules.
