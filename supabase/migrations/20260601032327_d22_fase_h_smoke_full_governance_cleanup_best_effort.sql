-- Best-effort cleanup: skip rule deletion (append-only group_rule_versions
-- guard cannot be bypassed without superuser privileges). Smokes from
-- multiple runs accumulate archived test rules in the sandbox; that is
...
BEGIN
  ...
  -- Cleanup (best-effort; rules linger as archived because group_rule_versions is append-only)
  BEGIN
  ...
  detail := 'deleted '||COALESCE(array_length(v_decision_ids,1),0)||' decisions, '
         ...
         || COALESCE(array_length(v_rule_ids,1),0)||' rules left archived (append-only versions)';
  RETURN NEXT;
END;
$$;
