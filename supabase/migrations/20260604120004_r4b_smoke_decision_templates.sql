...
-- =============================================================================
-- _smoke_r4b_decision_templates — verifies R.4B deliverables end-to-end.
-- =============================================================================
-- Asserted cases:
--   C1  decision_templates_catalog has 12 templates with expected execution_kinds
--   C2  decisions.template_key column + idx_decisions_template index exist
--   C3  create_decision(p_template_key='generic') sets template_key + inherits
--       default_voting_model from template when caller omits it
--   C4  create_decision with unknown template_key raises P0002
--   C5  generic template execute → effects = [{type:noop}]
--   C6  archive_resource template execute → resources.archived_at set
--   C7  archive_rule template execute → rules.status='archived'
--   C8  grant_resource_right template execute → resource_rights row created
--   C9  unimplemented kind (admit_member) raises errcode 0A000
--   C10 back-compat: decision without template_key still executes (effects=[])
--   C11 idempotency: re-executing already-executed decision returns already_executed=true
-- =============================================================================
...
begin
  ...
  -- ---------------------------------------------------------------------------
  -- C1: catalog seeded with 12 templates including the expected execution_kinds
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- C2: decisions.template_key + idx exist
  -- ---------------------------------------------------------------------------
  if not exists (
  ...
  -- ---------------------------------------------------------------------------
  -- C3: create_decision(template_key=generic) — inherits default_voting_model
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- C4: unknown template_key raises P0002
  -- ---------------------------------------------------------------------------
  v_caught := false;
  begin
  ...
  -- ---------------------------------------------------------------------------
  -- C5: generic execute → effects=[noop]
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- C6: archive_resource → resources.archived_at set
  -- ---------------------------------------------------------------------------
  v_resource_id := (public.create_resource(
    v_familia, '_smoke_r4b Recurso', 'asset', null,
  ...
  -- ---------------------------------------------------------------------------
  -- C7: archive_rule → rules.status='archived'
  -- ---------------------------------------------------------------------------
  ...
  values
    (gen_random_uuid(), v_familia, v_person, 'norm', 'active',
     '_smoke_r4b rule to archive', 'event.created', '{}'::jsonb, '[]'::jsonb, 'info')
  ...
  -- ---------------------------------------------------------------------------
  -- C8: grant_resource_right → resource_rights row inserted
  -- ---------------------------------------------------------------------------
  -- Need a non-archived resource for this branch. Reuse a fresh one.
  v_resource_id := (public.create_resource(
    v_familia, '_smoke_r4b Recurso Grant', 'asset', null,
    null, null, '{}'::jsonb, null
  )->>'resource_id')::uuid;
  ...
  -- ---------------------------------------------------------------------------
  -- C9: unimplemented kind raises 0A000 (admit_member)
  -- ---------------------------------------------------------------------------
  v_result := public.create_decision(
    p_context_actor_id => v_familia,
    p_decision_type    => 'governance',
  ...
  -- Cleanup: decision stayed at status=approved after failed execute (txn-local rollback inside BEGIN..EXCEPTION)
  delete from public.decisions where id = v_decision_id_5;

  -- ---------------------------------------------------------------------------
  -- C10: back-compat — decision without template_key still executes
  -- ---------------------------------------------------------------------------
  v_result := public.create_decision(
    p_context_actor_id => v_familia,
    p_decision_type    => 'generic',
  ...
  -- effects should be empty array for a generic decision with no winning option
  if v_result->'effects' <> '[]'::jsonb then
    raise exception 'r4b C10b: legacy execute unexpected effects %', v_result->'effects';
  end if;

  -- ---------------------------------------------------------------------------
  -- C11: idempotency — re-executing returns already_executed=true
  -- ---------------------------------------------------------------------------
  v_result := public.execute_decision(v_decision_id_6);
  ...
  delete from public.resource_rights
   where source_decision_id in (v_decision_id, v_decision_id_2, v_decision_id_3,
                                v_decision_id_4, v_decision_id_6);
  delete from public.rules where id = v_rule_id;
  delete from public.resources where context_actor_id = v_familia;
