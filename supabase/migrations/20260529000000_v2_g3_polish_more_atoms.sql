-- 20260529000000 — V2-G3 polish: expand atom surface 30% → 80% +
-- engine→votos bridge.
--
-- DATA (idempotente):
...
--   + UPDATE compatibility arrays en triggers existentes
--
-- CODE: _rule_eval_predicate + _rule_eval_dispatch ganan ramas para
-- los nuevos kinds. ELSIF hardcoded sigue siendo manejable a 15
-- atoms; el refactor a handler registry (rule_action_handlers table-
-- driven) queda para V3.
--
-- consequence.start_vote es el puente engine→votos. Doctrina: todo
-- lo que cambia autoridad va a voto; todo lo que aplica autoridad
-- existente es engine. Cuando una regla detecta un caso que requiere
-- deliberación, escala vía start_vote en vez de aplicar consecuencia
-- automática.
...
VALUES
   ...
   jsonb_build_object(
     ...
     'fields', jsonb_build_array(
       ...
       jsonb_build_object('key','charge_kind','type','enum','required',true,
                         'enum', jsonb_build_array('quota','buy_in','fee'),'label','Tipo de cobro'),
  ...
  ('consequence.start_vote', 'consequence',
   'Escalar a votación',
   'Arranca una decisión del grupo. Puente engine→votos: cuando la regla detecta un caso que requiere deliberación humana, en vez de aplicar consecuencia automática, llama al grupo a votar.',
   jsonb_build_object(
     'action', 'start_vote',
     'execution', 'sync',
...
-- ============================================================
-- 2) Update compatibility arrays en triggers preexistentes.
-- ============================================================
UPDATE public.rule_shapes_catalog SET schema = schema
  || jsonb_build_object(
       'compatible_conditions',
       jsonb_build_array('condition.actor_role_in','condition.amount_above','condition.amount_between'),
...
UPDATE public.rule_shapes_catalog SET schema = schema
  || jsonb_build_object(
       'compatible_conditions',
       jsonb_build_array('condition.actor_role_in','condition.target_self','condition.target_role_in'),
       'compatible_consequences',
...
-- ============================================================
-- 3) _rule_eval_predicate +3 kinds (amount_between, target_role_in, is_first_offense)
-- ============================================================
CREATE OR REPLACE FUNCTION public._rule_eval_predicate(
  p_condition_tree jsonb, p_event public.group_events
) RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE
  v_kind text; v_fields jsonb;
  v_actor_membership uuid; v_target_user_id uuid; v_target_membership uuid;
  v_actor_roles text[]; v_target_roles text[];
  v_required_roles jsonb; v_role text; v_match boolean := false;
  v_event_amount numeric; v_threshold numeric; v_amount_min numeric; v_amount_max numeric;
  v_target uuid; v_only_self boolean;
  v_lookback int; v_prior_count int;
BEGIN
  ...
  IF v_kind = 'condition.actor_role_in' THEN
    ...
    IF v_required_roles IS NULL OR jsonb_typeof(v_required_roles) <> 'array' THEN
      RETURN jsonb_build_object('passed', false, 'reason', 'missing_roles', 'kind', v_kind); END IF;
    ...
    IF v_actor_membership IS NULL THEN
      RETURN jsonb_build_object('passed', false, 'reason', 'actor_not_member', 'kind', v_kind); END IF;
    SELECT COALESCE(array_agg(r.key), ARRAY[]::text[]) INTO v_actor_roles
      FROM public.group_member_roles mr JOIN public.group_roles r ON r.id = mr.role_id
     WHERE mr.membership_id = v_actor_membership;
    FOR v_role IN SELECT jsonb_array_elements_text(v_required_roles) LOOP
      IF v_role = ANY(v_actor_roles) THEN v_match := true; EXIT; END IF; END LOOP;
  ...
  ELSIF v_kind = 'condition.target_role_in' THEN
    ...
    IF v_target_user_id IS NULL THEN
      v_target_user_id := NULLIF(p_event.payload->>'target','')::uuid; END IF;
    IF v_target_user_id IS NULL THEN
      RETURN jsonb_build_object('passed', false, 'reason', 'no_target_in_payload', 'kind', v_kind); END IF;
    ...
    IF v_target_membership IS NULL THEN
      SELECT id INTO v_target_membership FROM public.group_memberships
       WHERE group_id = p_event.group_id AND id = v_target_user_id; END IF;
    IF v_target_membership IS NULL THEN
      RETURN jsonb_build_object('passed', false, 'reason', 'target_not_member', 'kind', v_kind); END IF;
    SELECT COALESCE(array_agg(r.key), ARRAY[]::text[]) INTO v_target_roles
      FROM public.group_member_roles mr JOIN public.group_roles r ON r.id = mr.role_id
     WHERE mr.membership_id = v_target_membership;
    FOR v_role IN SELECT jsonb_array_elements_text(v_required_roles) LOOP
      IF v_role = ANY(v_target_roles) THEN v_match := true; EXIT; END IF; END LOOP;
    RETURN jsonb_build_object('passed', v_match,
  ...
  ELSIF v_kind = 'condition.amount_between' THEN
    ...
    RETURN jsonb_build_object('passed', v_match,
      ...
      'kind', v_kind,
      'evaluated_value', jsonb_build_object('event_amount', v_event_amount,
  ...
  ELSIF v_kind = 'condition.target_self' THEN
    ...
    IF NOT v_only_self THEN
      RETURN jsonb_build_object('passed', true, 'reason', 'self_check_disabled', 'kind', v_kind); END IF;
  ...
  ELSIF v_kind = 'condition.is_first_offense' THEN
    ...
    RETURN jsonb_build_object('passed', v_match,
      ...
      'kind', v_kind,
      'evaluated_value', jsonb_build_object('prior_count', v_prior_count,
...
-- ============================================================
-- 4) _rule_eval_dispatch +2 kinds (create_pool_charge, start_vote)
-- ============================================================
CREATE OR REPLACE FUNCTION public._rule_eval_dispatch(
  p_action jsonb, p_event public.group_events, p_rule_version_id uuid
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE
  v_kind text := p_action->>'kind';
  v_fields jsonb := COALESCE(p_action->'fields', '{}'::jsonb);
  v_target_user_id uuid; v_target_membership uuid;
  v_severity int; v_message text; v_audience text;
  v_new_state text; v_reason text;
  v_sanction_id uuid; v_error text; v_notified int;
  v_amount numeric; v_currency text; v_charge_kind text; v_obligation_id uuid;
  v_decision_id uuid; v_title text; v_decision_type text; v_method text;
  v_closes_at timestamptz; v_use_entity boolean; v_ref_kind text; v_ref_id uuid;
BEGIN
  v_target_user_id := COALESCE(
    NULLIF(p_event.payload->>'target_user_id','')::uuid, p_event.actor_user_id);
  SELECT id INTO v_target_membership FROM public.group_memberships
   WHERE group_id = p_event.group_id AND user_id = v_target_user_id;
  ...
  IF v_kind = 'consequence.issue_sanction' THEN
    ...
    IF v_target_membership IS NULL THEN
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'failed', 'error', 'target_membership_not_found'); END IF;
    BEGIN
      v_sanction_id := public.issue_sanction(
        ...
        p_client_id => NULL);
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'emitted', 'target_id', v_sanction_id, 'severity', v_severity);
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'failed', 'error', v_error); END;

  ELSIF v_kind = 'consequence.set_membership_state' THEN
    v_new_state := v_fields->>'new_state'; v_reason := v_fields->>'reason';
    IF v_target_membership IS NULL THEN
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'failed', 'error', 'target_membership_not_found'); END IF;
    BEGIN
      ...
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'emitted', 'target_id', v_target_membership, 'new_state', v_new_state);
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'failed', 'error', v_error); END;
  ...
  ELSIF v_kind = 'consequence.send_notification' THEN
    ...
    BEGIN
      ...
      ELSIF v_audience = 'group' THEN
        WITH ins AS (
          ...
          SELECT p_event.group_id, gm.user_id, 'rule_consequence',
                 jsonb_build_object('rule_version_id', p_rule_version_id, 'message', v_message,
                                    'source_event_id', p_event.uuid_id)
      ...
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'async',
        'status', 'emitted', 'audience', v_audience, 'recipients', v_notified);
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'async',
        'status', 'failed', 'error', v_error); END;
  ...
  ELSIF v_kind = 'consequence.create_pool_charge' THEN
    ...
    v_charge_kind := v_fields->>'charge_kind'; v_reason := v_fields->>'reason';
    IF v_target_membership IS NULL THEN
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'failed', 'error', 'target_membership_not_found'); END IF;
    IF v_amount IS NULL OR v_amount <= 0 THEN
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'failed', 'error', 'amount_required'); END IF;
    IF v_charge_kind NOT IN ('quota','buy_in','fee') THEN
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'failed', 'error', 'invalid_charge_kind'); END IF;
    BEGIN
      v_obligation_id := public.record_pool_charge(
        p_group_id => p_event.group_id, p_target_membership_id => v_target_membership,
        p_amount => v_amount, p_unit => v_currency, p_charge_kind => v_charge_kind,
        p_reason => COALESCE(v_reason, 'Generado por regla con engine'),
        p_mandate_id => NULL, p_client_id => NULL);
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'emitted', 'target_id', v_obligation_id,
        'amount', v_amount, 'currency', v_currency, 'charge_kind', v_charge_kind);
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_error = MESSAGE_TEXT;
      RETURN jsonb_build_object('kind', v_kind, 'execution', 'sync',
        'status', 'failed', 'error', v_error); END;
  ...
  ELSIF v_kind = 'consequence.start_vote' THEN
    ...
    v_ref_kind := NULL; v_ref_id := NULL;
    IF v_use_entity AND p_event.entity_kind IS NOT NULL AND p_event.entity_id IS NOT NULL THEN
      v_ref_kind := p_event.entity_kind; v_ref_id := p_event.entity_id; END IF;
    BEGIN
      v_decision_id := public.start_vote(
        ...
        p_opens_at => now(), p_closes_at => v_closes_at,
        p_threshold_pct => NULL, p_quorum_pct => NULL,
        p_committee_only => false,
        p_reference_kind => v_ref_kind, p_reference_id => v_ref_id,
        p_options => NULL,
