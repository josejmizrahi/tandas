-- 00113_resolve_governance_capability_enable.sql
-- Extend governance to handle `capability.enable`:
--
--   1. resolve_governance: admin_only branch dispatches to
--      Permission.modifyGovernance for capability.enable (capabilities
--      reshape what the group can do — treating them as governance-
--      adjacent rather than rule-adjacent. Founders + admins-with-
--      modifyGovernance toggle features; the doctrine line is "Group
--      Rules don't create features, but they decide who turns features
--      on").
--      Legacy fallback: capability.enable defaults to admin_only —
--      there is no legacy whoCan* field for it; the current
--      set_group_module RPC is admin-gated by RLS so this preserves
--      existing behaviour.
--   2. seed_default_group_policies: write a `capability.enable` row
--      with policy_type='admin_only'. Founders + custom admin roles
--      keep their direct toggle path; non-admins fall through to vote.
...
begin
  ...
  -- capability.enable has no legacy fallback. If we reach here the
  -- group's missing its policy row (top-up below + on-create trigger
  -- prevent that), so we'd return no_policy. The seeder defaults it
  -- to admin_only so the iOS side reads the row from group_policies
  -- on every group going forward.
...
-- =============================================================================
-- Extend seed_default_group_policies with capability.enable (default admin_only)
-- =============================================================================
...
begin
  ...
  -- expense.create — default direct
  insert into public.group_policies (group_id, policy_type, target_action, target_scope, approval_config, created_by, priority)
  values (p_group_id, 'direct', 'expense.create', 'group', '{}'::jsonb, p_created_by, 100)
  on conflict do nothing;

  -- capability.enable — default admin_only (preserves current
  -- set_group_module behavior where only admins toggle features).
  insert into public.group_policies (group_id, policy_type, target_action, target_scope, approval_config, created_by, priority)
...
begin
  for v_group in
    ...
    where not exists (
      ...
      where p.group_id = g.id and p.target_action = 'capability.enable'
    )
  loop
