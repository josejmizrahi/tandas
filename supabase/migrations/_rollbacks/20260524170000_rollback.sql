-- Rollback for 20260524170000_in_kind_contribution_flag.sql.
-- Drops the 8-arg fund_contribute / 7-arg contribute_to_shared_money
-- overloads added by mig 00364, restoring the pre-mig signatures.
-- Any ledger_entries.metadata.in_kind=true rows already written remain
-- as inert annotation — no projection reads them and the column is
-- harmless after rollback.

drop function if exists public.contribute_to_shared_money(uuid, bigint, text, text, uuid, uuid, boolean);
drop function if exists public.fund_contribute(uuid, bigint, text, text, uuid, uuid, uuid, boolean);

-- Restore the pre-mig-00364 7-arg fund_contribute.
...
-- Restore the pre-mig-00364 6-arg contribute_to_shared_money.
