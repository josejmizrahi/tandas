-- Rollback 00128 — restore previous create_group_with_admin body.
-- Doesn't undo the backfill UPDATE (those rows stay with their new
-- roles jsonb — re-adding 'founder' is the correct state anyway, the
-- previous code path just didn't write it).
...
begin
  ...
  if p_base_template is not null and length(trim(p_base_template)) > 0 then
    select t.config into template_config
      from public.templates t where t.id = p_base_template limit 1;
  end if;
  ...
  insert into public.groups (
    ...
    p_base_template, p_cover_image_name,
    resolved_active_modules, resolved_governance, resolved_settings,
    resolved_category, resolved_initials, uid
  ) returning * into g;

  -- Pre-Tier-0.5: no roles jsonb on founder insert.
