-- 00112_rollback.sql
-- Reverts resolve_governance + seed_default_group_policies to the
-- post-00111 shape (member.remove handled, expense.create not).
-- Group policy rows of target_action='expense.create' are left in place
-- — same pattern as 00111. Drop manually if you really want them gone:
--   delete from public.group_policies where target_action = 'expense.create';
