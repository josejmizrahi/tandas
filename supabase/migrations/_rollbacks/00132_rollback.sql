-- Rollback 00132 — drop next_host_for_series and restore 00129 body of
-- build_resource_from_draft (no series metadata merge of capability_configs).

drop function if exists public.next_host_for_series(uuid, int);
...
comment on function public.build_resource_from_draft(uuid, text, jsonb, text[], jsonb, jsonb, jsonb) is
  'Atomic ResourceWizard submit. v2 (00129): for events, threads p_capability_configs->rsvp->>deadline as p_rsvp_deadline into create_event_v2 so the wizard''s choice materializes in events.rsvp_deadline (and thus is what emit-deadline-events reads). Falls back to T-4h when absent or malformed.';
