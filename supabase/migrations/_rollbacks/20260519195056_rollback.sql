-- Rollback for 20260519195056_issue_manual_fine_client_id_idempotency.sql.
-- Restores the pre-V1-03 issue_manual_fine body (no p_client_id, no
-- idempotency). WARNING: re-tap by admin will again create duplicate
-- fines + duplicate ledger entries. Emergency revert only.
--
-- The 7-arg overload (with p_client_id) is left intact — drop it first
-- via mig 00354's rollback if you need to fully revert.
