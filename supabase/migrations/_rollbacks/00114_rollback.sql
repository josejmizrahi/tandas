-- 00114_rollback.sql
-- Reverts to post-00113 state (no member.invite handling). Existing
-- rows of target_action='member.invite' stay in place — drop manually
-- if you want a clean rollback:
--   delete from public.group_policies where target_action = 'member.invite';

-- (For brevity the rollback recreates resolve_governance + seed without
-- the member.invite branches; identical to 00113 final state.)

create or replace function public.resolve_governance(
  p_group_id uuid, p_actor_user_id uuid, p_target_action text,
  p_target_payload jsonb default '{}'::jsonb
) returns jsonb
language plpgsql stable security definer set search_path = public
as $$
declare
  v_is_member boolean; v_policy public.group_policies%rowtype; v_policy_found boolean := false;
  v_quorum int; v_threshold int; v_duration int; v_governance jsonb; v_who text; v_created_by uuid;
...
begin
  ...
  if found then
    ...
    if v_policy.policy_type = 'direct' then return jsonb_build_object('decision', 'allowed');
      ...
      v_required_permission := case when p_target_action like 'rule.%' then 'modifyRules'
