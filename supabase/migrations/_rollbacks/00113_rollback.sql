-- 00113_rollback.sql
-- Reverts to the post-00112 state: resolve_governance without the
-- capability.enable permission dispatch; seed_default_group_policies
-- without the capability.enable insert. Existing rows of
-- target_action='capability.enable' stay in place — drop manually if
-- a clean rollback is required:
--   delete from public.group_policies where target_action = 'capability.enable';
