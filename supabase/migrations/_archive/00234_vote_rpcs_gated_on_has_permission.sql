...
--
-- Sibling of mig 00232 (fines). Same governance-review item #1
-- ("two auth models coexisting"), votes slice.
--
-- Bug
-- ===
-- start_vote and cast_vote relied on group membership only as their
-- authorization gate. The role-based permission model (mig 00063 +
-- 00228) defines `createVotes` and `castVote` permissions, but the
-- vote RPCs never consulted them. Result: a custom role that revokes
-- voting rights (e.g. a future `observer` member type) can't actually
-- prevent the holder from opening or casting votes — the iOS UI would
-- hide the buttons but the RPCs accept calls anyway.
--
-- Fix
-- ===
-- Add has_permission(...) checks after the existing membership
-- lookup. Membership check stays first for a clearer error message
-- ("not a member" vs "permission required").
--
-- Permissions used: both ship in the V1 catalog (mig 00063 column
-- default). Default founder.permissions includes createVotes; default
-- member.permissions includes createVotes + castVote. All current
-- active members in prod have 'member' in group_members.roles
-- (verified 2026-05-17, 8/8), so this is a no-op for existing users
-- and a guardrail for future custom roles.
--
-- Out of scope
-- ============
-- - cancel_vote: gate is "creator of the vote" (mig 00222). Admin
--   override would need a new `cancelVote` permission; not included
--   in this slice.
-- - finalize_vote: server-side cron path (no auth.uid()). State
--   machine gates by vote.status + closes_at; idempotent. No auth
--   check appropriate.
--
-- Idempotent: CREATE OR REPLACE swaps the function body atomically.
-- Body otherwise identical to deployed versions (start_vote: mig 00130
-- + drift; cast_vote: mig 00163).
--
-- Rollback: _rollbacks/00233_rollback.sql restores prior bodies.

-- =========================================================
-- 1. start_vote — has_permission('createVotes')
-- =========================================================
...
-- =========================================================
-- 2. cast_vote — has_permission('castVote')
-- =========================================================
