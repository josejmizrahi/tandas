...
--
-- Closes governance-review item #1 (Track A "two auth models coexisting"):
-- issue_manual_fine, void_fine, pay_fine still gated on is_group_admin
-- despite the role-based permission model shipped in mig 00063 +
-- aggregated in mig 00228. Result: a custom role like `treasurer` that
-- legitimately needs fine-issuing authority can't get it without being
-- promoted to admin/founder. This breaks the promise of templates
-- declaring custom roles with surgical permissions.
--
-- Catalog gap (fixed in Permission.swift sibling commit):
--   - issueFine     (V1): authority to manually fine another member.
--   - markFinePaid  (V1): authority to record payment on behalf of a
--                          fined member (external/off-app payments).
--   `voidFine` already shipped in mig 00063.
--
-- This migration:
--   1. Extends the column DEFAULT for new `public.groups.roles` to
--      include issueFine + markFinePaid in `founder.permissions`.
--   2. Backfills existing groups: adds the two permissions to every
--      group's `roles.founder.permissions` array if missing. Idempotent
--      via distinct-unnest, safe to re-run.
--   3. Backfills `public.templates.config.defaultRoles.founder
--      .permissions` for templates that override the default seed
--      (today: `shared_resource`, mig 00066). Same idempotent merge.
--   4. Replaces the auth check inside issue_manual_fine, void_fine,
--      pay_fine with has_permission(group, uid, '<perm>').
--      - pay_fine keeps the self-pay branch: `f.user_id = auth.uid()`
--        always allowed (you can always pay your own fine without
--        needing markFinePaid).
--
-- Behaviour for existing founders: zero change. They start with the
-- two new permissions in their role definition courtesy of step 2/3,
-- and has_permission(group, uid, 'X') returns true. All existing flows
-- keep working.
--
-- Behaviour for the legacy `admin` text role (group_members.role): the
-- 00228 has_permission already aliases legacy 'admin' to 'founder',
-- so members on legacy admin keep working without backfill.
--
-- Idempotent: all steps re-runnable (jsonb merge is set-union; CREATE
-- OR REPLACE FUNCTION is atomic swap).
--
-- Rollback: see _rollbacks/00232_rollback.sql (restores
-- is_group_admin gating; does not touch the catalog backfill since it
...
-- =========================================================
-- Set-union merge: rebuild the founder.permissions array as the
-- DISTINCT union of (current values) ∪ {issueFine, markFinePaid}.
-- Groups whose `roles` jsonb doesn't have a `founder` entry are
-- left untouched (defensive — only applies if some migration ahead
-- intentionally removed system roles).
...
-- =========================================================
-- 4. issue_manual_fine — has_permission('issueFine')
-- =========================================================
-- Body otherwise identical to mig 00155. Only the auth gate changed.
-- Note on signature: deployed function in prod (mig 00155 + post-apply
-- drift) carries `p_rule_id uuid DEFAULT NULL`, not the bare `uuid` in
-- 00155's source. CREATE OR REPLACE refuses to drop parameter defaults,
-- so we match the deployed shape. Semantically identical.
...
-- =========================================================
-- 5. void_fine — has_permission('voidFine')
-- =========================================================
-- Body otherwise identical to mig 00142. Only the auth gate changed.
...
-- =========================================================
-- 6. pay_fine — self-pay OR has_permission('markFinePaid')
-- =========================================================
-- Body otherwise identical to mig 00146. Only the admin branch of
-- the auth gate changed; the self-pay branch (f.user_id = auth.uid())
-- is preserved unchanged so users never need a permission to pay
-- their own fine.
