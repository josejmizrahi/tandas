-- 00080 — Fix create_event_v2 for post-BigBang schema.
--
-- The RPC referenced `g.rotation_enabled` (dropped by 00078). Any iOS
-- call into the wizard / CreateEventView surfaced as "no pudo crear el
-- evento" because the function bailed at runtime with "column
-- rotation_enabled does not exist".
--
-- Post BigBang, rotation lives as a capability block on the rotating_host
-- module. Presence of `rotating_host` in `groups.active_modules` is the
-- new proxy for "auto-pick the next host".
--
-- Other RPCs that still touch dropped columns (create_event, pay_fine,
-- roll_event_series, update_group_config) get fixed in follow-up
-- migrations as their entry points get exercised in the new architecture.
...
begin
  ...
  -- Rotation is a capability block now. The rotating_host module
  -- declares it; presence in active_modules is the runtime gate.
  v_rotation_on := coalesce(g.active_modules ? 'rotating_host', false);
