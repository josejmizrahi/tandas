...
begin
  ...
  if p_host_id = p_creator_id then
    raise exception 'test_hostAssigned_atom_emission: p_host_id and p_creator_id must differ';
  end if;
  ...
  begin
    ...
    begin
      ...
      select count(*) into v_system_event_count
      ...
      select member_id, payload
        into v_member_id_in_atom, v_payload_in_atom
        from public.system_events
       where resource_id = v_resource_id
         and event_type = 'hostAssigned'
       limit 1;
