-- 00101 — Atomic ResourceWizard submit RPC.
--
-- Founder framing 2026-05-11 #5: "iOS no debería orquestar N writes
-- críticos uno por uno". Today's `EventResourceBuilder.build(draft)`
-- and `AssetResourceBuilder.build(draft)` do 3-6 sequential round-
-- trips (create_event_v2 + create_resource_series + setSeriesId +
-- per-capability insert + per-rule create_resource_rule). Partial
-- failure leaves orphan state — an event without its rules, a series
-- without its first occurrence.
--
-- This RPC takes the FULL draft shape and runs everything in one
-- function-scope transaction. On any internal raise, the entire
-- write rolls back atomically.
--
-- Polymorphic by resource_type: an internal CASE creates the
-- resource row via the matching create_* helper. Capabilities, rules,
-- and the optional series are handled uniformly (those tables are
-- already polymorphic over resource_type).
--
-- iOS callers replace their build() orchestration with a single
-- supabase.rpc("build_resource_from_draft", …) call.
...
begin
  ...
  -- =========================================================
  -- Must land before the resource so resources.series_id is settable
  -- in the same transaction. The series row carries the pattern
  -- jsonb; per-occurrence resource rows reference it via series_id.
  ...
  when 'event' then
    ...
    -- create_event_v2 already inserts into public.events AND
    -- dual-writes into public.resources via the trigger from mig
    -- 00039. We then look up the resource_id by the event id.
    declare
      v_event_id uuid;
    begin
  ...
  -- =========================================================
  -- 4. Enable capabilities (idempotent — trigger may have seeded
  -- some defaults already; ON CONFLICT keeps us safe)
  -- =========================================================
  ...
  -- =========================================================
  -- 5. Seed initial rules (each scoped to this resource)
  -- =========================================================
  -- Shape per element: { slug, name, trigger, conditions, consequences }.
  -- We bypass create_resource_rule's host check because the resource
  -- was just created in this same transaction — the caller has implicit
  -- authoring rights over a resource they just built.
...
revoke execute on function public.build_resource_from_draft(uuid, text, jsonb, text[], jsonb, jsonb, jsonb) from public, anon;
grant  execute on function public.build_resource_from_draft(uuid, text, jsonb, text[], jsonb, jsonb, jsonb) to authenticated;
