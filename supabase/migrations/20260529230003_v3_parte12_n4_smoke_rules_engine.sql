...
--
-- Cobertura:
--   - rule_shapes_catalog tiene las 3 shapes necesarias.
--   - validate_rule_shape happy path → valid=true.
--   - validate_rule_shape con shape_key inexistente → valid=false.
--   - create_engine_rule end-to-end (incluye publish_rule_version internamente).
--   - Engine eval matched=true cuando trigger+condition cumplen → notifications_outbox row 'rule_consequence'.
--   - Engine eval matched=false cuando condition no cumple (audit-only).
--
...
--
-- Drifts vs spec §N.4 capturados aquí:
