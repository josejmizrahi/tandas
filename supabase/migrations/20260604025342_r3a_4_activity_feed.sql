-- ============================================================================
-- R.3A.4 — activity_feed(p_actor_id?, p_limit?)
-- ============================================================================
-- Devuelve un feed personalizado para un actor. Une 3 fuentes de actividad:
--   (a) Subscriptions activas del actor (context/actor/resource/decision/event/obligation)
--   (b) Ownership: rights activos con right_kind='own' sobre recursos
--   (c) Membership: contextos donde el actor es miembro activo
--
-- Cada evento se anota con:
--   source: 'subscription' | 'ownership' | 'membership'
--   subscription_type: cuando source='subscription' (watch/follow/stakeholder/audit/owner_interest)
--   score: peso por relevancia (mayor = más arriba)
--
-- Pesos (founder lock R.3A):
--   owner_interest = 100
--   ownership      = 90
--   stakeholder    = 80
--   membership     = 70
...
--   follow         = 30
--
-- Ranking final: ORDER BY score DESC, occurred_at DESC.
-- Read-only. NO emite eventos. NO muta nada.
-- ============================================================================
...
revoke all on function public.activity_feed(uuid, int) from public, anon;
grant execute on function public.activity_feed(uuid, int) to authenticated, service_role;

comment on function public.activity_feed(uuid, int) is
'R.3A: feed personalizado union(subscriptions + ownership + membership). NO red social — señales relevantes solamente.';
