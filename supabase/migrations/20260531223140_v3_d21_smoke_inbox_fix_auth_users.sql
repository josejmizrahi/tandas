-- D.21B fix — _smoke_inbox needs auth.users + profiles rows so FK passes
...
begin
  ...
  insert into public.profiles (id, display_name) values
    (v_uid, 'Inbox Smoke Self'), (v_other, 'Inbox Smoke Other');
  ...
  -- cleanup (cascade from auth.users will drop profiles + outbox via FKs)
