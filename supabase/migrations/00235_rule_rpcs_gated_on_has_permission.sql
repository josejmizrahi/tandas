-- 00234 — Rule RPCs gated on has_permission (Permission catalog v2).
--
-- Sibling of 00232 (fines) + 00233 (votes). Same item #1 ("two auth
-- models coexisting"), rules slice — partial.
--
-- In scope
-- ========
-- - publish_rule_version (mig 00182)  → has_permission('modifyRules')
-- - seed_template_rules  (mig 00075)  → has_permission('modifyRules')
--
-- Out of scope
-- ============
-- - create_initial_rule (mig 00122): already routed through
--   public.resolve_governance(group, uid, 'rule.create', ...). That
--   path supports {allowed, vote_required, denied, admin_only} which
--   is the better long-term gate. Rewiring resolve_governance to
--   consult has_permission as the primary check (instead of
--   is_group_admin as the fallback) is a deeper change that deserves
--   its own slice.
--
-- The `modifyRules` permission already ships in the V1 catalog
-- (groups.roles.founder.permissions, mig 00063). All 8 existing
-- groups have it — no observable change for founders. Enables
-- custom roles (e.g. a "rule_editor" role given to a co-admin) to
-- get this surgical authority without being promoted to founder.
--
-- Idempotent: CREATE OR REPLACE swaps the function bodies atomically.
-- Bodies otherwise identical to deployed mig 00182 / 00075.
--
-- Rollback: _rollbacks/00234_rollback.sql restores prior bodies.

-- =========================================================
-- 1. publish_rule_version — has_permission('modifyRules')
-- =========================================================
...
begin
  ...
  -- mig 00234: swap is_group_admin → has_permission('modifyRules').
...
comment on function public.publish_rule_version(uuid, text, jsonb, jsonb, text, text) is
  'v2 (mig 00234): auth gate is has_permission(modifyRules) instead of is_group_admin. Body otherwise identical to mig 00182.';

-- =========================================================
-- 2. seed_template_rules — has_permission('modifyRules')
-- =========================================================
...
begin
  ...
  -- mig 00234: swap is_group_admin → has_permission('modifyRules').
  if not public.has_permission(p_group_id, uid, 'modifyRules') then
    raise exception 'modifyRules permission required' using errcode = '42501';
  end if;
...
comment on function public.seed_template_rules(text, uuid) is
  'v3 (mig 00234): auth gate is has_permission(modifyRules) instead of is_group_admin. Body otherwise identical to mig 00075.';
