-- R.5A.B.7.1 — Enriquece context_detail_descriptor con tres nuevos previews:
--   child_contexts_preview     := subcontextos (actor_relationships contains)
--   pending_invitations_preview := invites activos no expirados con cupos
--   money_preview.my_balance_by_currency := net por currency del caller
--
-- Slice additive: misma signature (uuid → jsonb). Los nuevos campos llegan
-- como arrays/objects en el JSON; iOS los renderea opcionalmente. Shape
-- back-compat: nada existente se mueve, sólo se agregan claves.
--
-- Doctrina: descriptor es la fuente única para Context Detail; el iOS V2
-- evita round-trips a context_children / list_invites / actor_money_balances
-- por separado. B.7.1 = mirror de B.6.1 (resource enrichment) para Context.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'member_count', (
             select count(*) from public.role_assignments ra2
              where ra2.role_id = r.id
           )
  ...
  -- B.7.1 — my_balance_by_currency: net del caller en este contexto
  select coalesce(jsonb_object_agg(b.currency, b.net_balance), '{}'::jsonb)
    into v_my_balance_by_currency
  ...
  v_money_preview := jsonb_build_object(
    'my_balance', null,
    'my_balance_by_currency', v_my_balance_by_currency,
    'open_settlements', (
      select count(*) from public.obligations
       where context_actor_id = p_context_actor_id and status='open'
  ...
  -- B.7.1 — child_contexts_preview: subcontextos via actor_relationships
  select coalesce(jsonb_agg(jsonb_build_object(
           'id', a.id,
           'display_name', a.display_name,
  ...
  -- B.7.1 — pending_invitations_preview: invites activos no expirados con cupos
  select coalesce(jsonb_agg(jsonb_build_object(
           'invite_id', ci.id,
           'code', ci.code,
           'max_uses', ci.max_uses,
           'used_count', ci.used_count,
           'expires_at', ci.expires_at,
           'created_at', ci.created_at
         ) order by ci.created_at desc), '[]'::jsonb)
    into v_pending_invitations_preview
    from public.context_invites ci
   where ci.context_actor_id = p_context_actor_id
