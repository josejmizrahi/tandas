-- ============================================================================
-- _smoke_r5_governance — fix: activity_events es append-only.
-- ============================================================================
-- La primera iteración (20260604150001) intentaba `delete from activity_events`
-- en el cleanup, que dispara el trigger _activity_events_append_only (42501).
-- No es necesario: todos los FKs de activity_events hacia actors/decisions son
-- ON DELETE SET NULL, así que el teardown de actores/decisiones limpia las
-- referencias sin borrar la fila. Esta es la versión canónica del smoke.
-- ============================================================================
...
begin
  ...
  -- C8: member B no puede fijar politicas
  perform set_config('request.jwt.claims', jsonb_build_object('sub', v_auth_b::text)::text, true);
  v_caught := false;
  begin
  ...
  -- C3: weighted voting por shares (B = 3 votos)
  perform set_config('request.jwt.claims', jsonb_build_object('sub', v_auth_a::text)::text, true);
  ...
  -- C4: delegacion (peso igualitario)
  perform set_config('request.jwt.claims', jsonb_build_object('sub', v_auth_a::text)::text, true);
  ...
  -- Cleanup (sin tocar activity_events: FKs ON DELETE SET NULL lo resuelven)
