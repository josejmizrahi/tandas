-- 20260528060300 — V2-G3 sub-slice 4: predicate evaluator + consequence
-- dispatcher + integration en evaluate_rules_for_event.
--
-- Cierra el loop "reglas son políticas no texto": el evaluator ahora
-- corre la IF clause contra el evento real, dispatcha cada THEN según
-- atom.execution (sync inline vs async outbox), y persiste per-action
-- outcome detail en la fila de audit.
--
-- Doctrine §3: sync canonical vs async derived. issue_sanction +
-- set_membership_state son sync (PERFORM al canonical RPC en la misma
-- tx); send_notification es async (insert en notifications_outbox).
-- Doctrine §5: explainability. Cada fila ahora tiene
-- matched_predicate.outcome con {passed, reason, evaluated_value} y
-- actions_emitted con per-action {kind, execution, status, error?}.
--
-- Cycle handling: si cycle_detected=true (mismo rule_version_id en el
-- chain ancestral), la fila aún se inserta pero las consequences
-- NO se dispatchan. Doctrine "transparencia primero".
-- Failure handling: cada dispatch tiene su BEGIN/EXCEPTION; un
-- handler que falla NO rollbackea la evaluation. Se audita
-- {status:'failed', error}.
