-- 20260527130000 — group_mandates Foundation (Primitiva 23).
--
-- Primitiva 23 (Representación): mandatos revocables, scoped. Distinta
-- de roles (un rol es una posición persistente; un mandato es una
-- delegación específica con propósito y tiempo). Schema ya canónico
-- (00001 + 00040 expansions). CHECK constraints:
--   - principal_type ∈ ('group','committee','role','membership')
--   - mandate_type   ∈ ('speak','sign','vote','negotiate','spend',
--                       'represent','delegate','other')
--   - status         ∈ ('active','expired','revoked','fulfilled')
--
-- Foundation RPCs:
--   - group_mandates_active(p_group_id) — read, filtra revoked/expired
--   - grant_mandate(...)                — gated por mandates.grant
--   - revoke_mandate(p_mandate_id, p_reason) — gated por mandates.revoke

-- ===========================================================================
-- 1. READ: group_mandates_active
-- ===========================================================================
...
BEGIN
  ...
  -- Validate enums up-front so callers get our error strings rather
  -- than raw CHECK violations.
  ...
  -- principal_id required for non-group principals; ignored for group.
  ...
  -- Representative must point to a current active membership in the group.
  IF NOT EXISTS (
    SELECT 1 FROM public.group_memberships gm
