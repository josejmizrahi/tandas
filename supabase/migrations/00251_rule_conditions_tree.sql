...
--   - `compiled.shape_ids.conditions` lists every leaf id from anywhere
--     in the tree (flat array view of the tree's leaves) so downstream
--     consumers (capability checks, conflict signatures) keep working
--     without learning the tree shape.
...
begin
  ...
  -- Leaf — must have a non-empty `type` or `shape_id`. `config` may be
  -- absent / null / an object. Author input from the RPCs uses
  -- `shape_id`; the compiled snapshot uses `type` — accept either so
  -- the validator can run both pre- and post-compile.
...
-- =============================================================================
-- 2. compile_condition_tree — author shape_id → engine type transform.
-- =============================================================================
...
-- =============================================================================
-- 3. extract_condition_shape_ids — flat pre-order leaf id list.
-- =============================================================================
...
-- =============================================================================
-- 4. publish_rule_composition v6 — tree acceptance + validation.
-- =============================================================================
-- Same author surface as v5 (mig 00246) except `p_conditions` now accepts
-- EITHER a flat array of `{shape_id,config}` leaves OR a `{op,children}`
-- tree where each leaf is `{shape_id,config}`. The compiled
-- `rule_versions.compiled.conditions` mirrors the input shape so
-- `runRulesForEvent` can evaluate it directly.
...
comment on function public.publish_rule_composition(uuid, text, jsonb, jsonb, jsonb, jsonb, text, text, jsonb, uuid) is
  'v6 (§22.4): admin-only publish of a per-piece rule composition. p_conditions now accepts EITHER a flat array of {shape_id,config} leaves OR a {op,children} AND/OR/NOT tree. The compiled snapshot preserves the tree shape; compiled.shape_ids.conditions is always the flat pre-order leaf list so capability/conflict consumers stay unchanged. Validates the tree via validate_condition_node before persisting.';

-- =============================================================================
-- 5. bump_rule_version v5 — same tree acceptance + validation on edit-in-place.
-- =============================================================================
...
comment on function public.bump_rule_version(uuid, text, jsonb, jsonb, jsonb, text, jsonb, uuid, boolean) is
  'v5 (§22.4): edit-in-place rule version bump. Same surface as v4 (mig 00247) but p_conditions now accepts EITHER a flat array (legacy AND) or a {op,children} AND/OR/NOT tree. Validates via validate_condition_node before persisting. The compiled snapshot preserves the tree; compiled.shape_ids.conditions stays the flat pre-order leaf list.';
