...
--
-- visibility defaults to 'members' so the basic case stays one-tap.
...
BEGIN
  ...
  -- Permission + active-member gate (assert_permission joins on
  -- group_memberships.status='active').
  PERFORM public.assert_permission(p_group_id, 'reputation.record');

  -- Resolve caller's active membership id for actor attribution. If the
  -- caller isn't a member of the group (e.g. system-triggered runs),
  -- leave actor_membership_id NULL.
