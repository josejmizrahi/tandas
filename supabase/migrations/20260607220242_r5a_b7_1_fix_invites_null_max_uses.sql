-- R.5A.B.7.1 fix — pending_invitations_preview ignoraba invites con
-- max_uses=NULL (unlimited). NULL < anything == NULL (falsy) en SQL.
-- Patch local: solo cambia la cláusula del WHERE de v_pending_invitations_preview.
...
begin
  ...
  -- FIX: max_uses NULL = unlimited (NULL < N == NULL == falsy en SQL)
  select coalesce(jsonb_agg(jsonb_build_object(
           'invite_id', ci.id,
           'code', ci.code,
