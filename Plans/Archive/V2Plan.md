...
- [x] V2-G2 Decision types con outcome handlers — sub-slices 1-6 cerrados: propose surface (11 valores) + read surface (reference + outcome hint) + reference pickers para sanction / mandate / membership / rule_change / budget. Backend handlers cubiertos en mig 20260528030000 (membership + p_metadata) + mig 20260528040000 (rule_change archive/activate) + mig 20260528050000 (budget → group_obligations pool_charge inline con quota/buy_in/fee). iOS DecisionsStore.draftMetadata es composer typed que cubre los 3 paths. `rule_change` action='edit' (cambiar body/title) y proposer-side de dissolution quedan como follow-ups menores fuera del scope V2-G2.
- [x] V2-G3 Rule engine activation — epic cerrado: G3.1 atom catalog + shape builder + dry-run (`04976c71`) → G3.3 explainability columns (`30efadce`, mig `060200`) → G3.4 predicate evaluator + 3 consequence handlers (`904e9127`, mig `060300`) → G3.2 hook en 4 callsites (`7270e3d4`, mig `060400`) → G3.5 iOS Disparos feed (`cf9a8b8d`, mig `060500`) + polish atoms 30%→80% + engine→votos bridge (`6bb56c78`). 20 atoms vivos (8 triggers + 6 conditions + 6 consequences). 427/427 tests RuulCore. Simulador V3.
- [x] V2-G4 Sanction ↔ Money deep — sub-slice 1 partial payments (`e5042455`) + sub-slice 2 payment plans MVP (`36548366`, mig `20260529040000`). Sub-slice 3 (auto-pay from fund) y sub-slice 4 (convert to service post-vote) deferred V3 al cortar `v2.0.0-rc`.
...
- [x] V2-G8 Engine-driven UX — sub-slice 1 banner global en `GroupHomeFeedView` (`6f5481b3`) + sub-slice 2 "¿Por qué pasó esto?" sheet con provenance (`aedd1ae1`) + sub-slice 3 `EngineRuleDetailView` per-regla "Disparos recientes" (post-rc polish).
- [x] V2-G9 Vote weights por rol/contribución (`14a0eebd`) — incluye la UX de `ranked_choice` (drag-to-reorder en VoteSheet) y `weighted` (input numérico por opción) heredadas de V2-G1; backend nuevo: `cast_vote` ampliado con `p_weight` + RPC `cast_ranked_vote(p_decision_id, p_rankings jsonb)` + columna `weight_strategy` en `group_decisions`.
- [x] **V2 release tag** `v2.0.0-rc` — 2026-05-29 (`930d7fca` documenta el cierre)
