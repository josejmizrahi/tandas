...
- **+1 aplicada 2026-05-29 (PARTE 7c — group_governance_versions read RPC + iOS cable)**:
  - `20260529220001 v3_parte7_c_group_governance_versions_read_rpc` — RPC read `group_governance_versions(p_group_id, p_limit DEFAULT 20)` returns `TABLE(id, snapshot jsonb, effective_from, effective_until, set_by_user_id, set_by_display_name, source_decision_id, created_at)` ordenada `effective_from DESC`. Pre-join con `profiles.display_name`. Active-member gate + `SECURITY DEFINER + SET search_path = public, pg_catalog`. iOS shipped: `GroupGovernanceVersion` domain type + `GovernanceSnapshot` con keys del jsonb + `groupGovernanceVersions` en protocol + Supabase impl + mock + `CanonicalDecisionRulesRepository.history(groupId:limit:)` + `DecisionRulesStore.refreshHistory(...)` + `historySection` en `EditDecisionRulesView`. Smoke: shape correcto con 2 versions del grupo testigo (supermajority active + majority closed) hidratado con `Smoke User A` display_name ✓.
...
- `group_events_recent` ORDER BY `occurred_at DESC` **sin tie-break secundario `id DESC`** → ordering no determinístico cuando timestamps coinciden (caso real: múltiples eventos en una misma RPC tx). **CERRADO 230007 (batch 4)**.

- **+2 aplicadas 2026-05-29 (PARTE 12 batch 4 — tie-break fix + N.12)**:
  - `20260529230007 v3_parte12_fix_group_events_recent_tiebreak_and_n8_strict` — fix operativo: `group_events_recent` ORDER BY ahora `occurred_at DESC, id DESC`. Cursor pagination ahora determinística incluso con timestamps coincidentes (caso real: eventos same-tx). + Upgrade de N.8.4 a strict assertion `first_uuid = v_event_uuid_2`. iOS-facing API unchanged. Smoke memory_audit 5/5 strict verde.
  - `20260529230008 v3_parte12_n12_smoke_cross_tenant_guards` — `_smoke_cross_tenant_guards()` formaliza N.12 con 5 asserts cross-tenant RPC blockage: record_expense, group_events_recent, invite_member, set_decision_rules, assert_permission. A (active en group_x) operando sobre group_y → todas las RPCs raise. Verifica defensa en profundidad por capas (assert_member_of_group + assert_permission + RLS chain).

- **+2 aplicadas 2026-05-29 (PARTE 12 batch 5 — hot-fix V3-A4b emisor + N.5 ampliada)**:
  - `20260529230009 v3_parte12_hotfix_emit_mandate_expiring_status_active` — hot-fix V3-A4b: `emit_mandate_expiring_events` filtraba `status='granted'` que NO existe en BD (grant_mandate inserta `status='active'`). El emisor nunca disparaba; memoria project_v3_block_a_shipped.md decía "✅ Idempotent" pero el filter principal estaba muerto. Rename mechanical al estado real, paralelo a hot-fix de start_vote. Smoke N.5.9 ahora verde tras fix.
  - `20260529230010 v3_parte12_n5_smoke_money_extended` — `_smoke_money_extended()` formaliza N.5 ampliada con 5 asserts: precheck hot-fix + N.5.5a reverse_transaction con dependent obligations bloqueado + N.5.5b reverse contribution OK + reversed_entry_id link + event + N.5.6 custom split_breakdown honra amounts custom (B=30 de 100) + N.5.9 emit_mandate_expiring_events idempotent (1st=1, 2nd=0, total event=1).

**Hallazgos PARTE 12 batch 5 / drifts capturados**:
- `group_mandates.status` whitelist real (observado): `active | revoked`. Doctrina `granted` mencionada en V3-A4b nunca aterrizó; el code path estaba muerto. Hot-fix aplicado: emit usa `active`.
...
- **+2 aplicadas 2026-05-29 (PARTE 12 batch 7 — drift sweep + N.13 meta-smoke permission keys audit)**:
  - `20260529230013 v3_parte12_hotfix_finalize_vote_membership_state_whitelist` — hot-fix #4: `finalize_vote` membership handler validaba `target_state IN ('active','suspended','expelled','inactive')` pero `set_membership_state` whitelist real es `active|suspended|left|banned|requested|invited`. iOS sigue mandando `expelled`/`inactive` como legacy. Resultado: votar expulsión RAISE silenciosamente y rollback toda la finalización. Hot-fix: agregar CASE mapping (`expelled`→`banned`, `inactive`→`left`) antes del IN list. Alinea IN list al whitelist real.
  - `20260529230014 v3_parte12_n14_smoke_dead_value_regression_guard` — `_smoke_dead_value_regression_guard()` con 5 asserts que vigilan re-introducción de los valores muertos cazados esta sesión: `'granted'` status, `'decisions.propose'` permission, `'sanction.review'` permission, `'free_form'` decision_type, llamadas a `set_membership_state` con `'expelled'`/`'inactive'` directos. Smoke se auto-excluye; N.14.5 tightened para aceptar el CASE WHEN input como legacy compat.

**Hallazgos PARTE 12 batch 7**:
- **Cuarto silent breakage cazado**: `finalize_vote` membership handler con whitelist `expelled`/`inactive` rota desde V1. Voto sobre expulsión nunca disparó set_membership_state correctamente. iOS sigue mandando `expelled`/`inactive` desde `MembershipDecisionTargetState` (legacy enum en GroupDecision.swift). CASE mapping en finalize_vote ahora traduce legacy → canonical antes del dispatch. Patron documentation pendiente: iOS debería migrar a `banned`/`left` directos en V3+ (TODO doctrinal, no rush — el mapping cubre legacy).
- **Pattern de los 4 silent breakages**: cada uno introducido por un autor distinto que asumió que el "intent key" aterrizaría en catalog/whitelist. Nadie cerró el loop. Sin smoke meta-audit (N.13 + N.14), el TTL de detection era "cuando un usuario real lo pegue". Ahora: PR-time, gracias a los smokes.

- **+3 aplicadas 2026-05-29 (PARTE 12 batch 8 — idempotency drift sweep + N.13.3)**:
  - `20260529230015 v3_parte12_hotfix_record_payout_idempotency_memo` — hot-fix #5: `record_payout` declaraba p_client_id y lo storeaba en `group_resource_transactions.client_id` (UNIQUE en (group_id, client_id)) pero NO hacía memo check al inicio. Retry → UNIQUE raise → client recibe error en vez del tx_id existente. No idempotent. Hot-fix: pattern canonical de memo lookup (paralelo a record_contribution).
  - `20260529230016 v3_parte12_hotfix_record_pool_charge_idempotency_memo` — hot-fix #6: `record_pool_charge` storeaba client_id en `metadata` jsonb (sin UNIQUE column). Retry → double-charge silente (cada call insertaba nueva obligation). **Severo bug financiero**. Hot-fix: memo lookup via `metadata->>'client_id'`.
  - `20260529230017 v3_parte12_n13_idempotency_dead_param_check` — N.13.3 nuevo assert: detecta RPCs SECURITY DEFINER con `p_client_id text` declarado pero NUNCA referenciado en body (param muerto). Bug del regex inicial: PG usa `\y` (no `\b`) para word boundary. Smoke 3/3 verde tras fix. **Whitelist temporal**: `submit_rsvp` + `submit_check_in` ambos tienen p_client_id 100% muerto — DEFERRED hot-fix pending founder doctrine sobre dónde almacenar el client_id (column nueva, jsonb, o remover param).

**Hallazgos PARTE 12 batch 8**:
- **5to + 6to silent breakages cazados**: `record_payout` (no idempotent, retry raises) + `record_pool_charge` (double-charge en retry — financiero). Ambos hot-fixed con pattern canonical de memo lookup.
- **2 dead-param drifts deferred**: `submit_rsvp` y `submit_check_in` declaran p_client_id pero el body lo ignora. Pending founder decision: ¿memo via column nueva en `group_rsvp_actions`/`group_check_in_actions`, via jsonb metadata, o remover param y dejar iOS retry-strategy via UI?
- **Total silent breakages cazados esta sesión: 6**. Pattern documentado en N.13: el TTL pre-meta-audit era O(usuario real). Post-N.13: PR-time.
