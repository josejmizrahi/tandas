...
- [~] V2-G2 Decision types con outcome handlers — sub-slices 1+2+3: propose surface (11 valores canónicos agrupados) + read surface (reference + outcome en DecisionDetailView) + propose-side reference picker (sanction_appeal → SanctionsStore picker; mandate_grant/revoke → MandatesStore picker; dissolution → hint). Backend `finalize_vote` ya dispatcha sanction/dispute/mandate_grant/mandate_revoke/dissolution; iOS expone "Afecta a…" + "Consecuencia". Falta sub-slice 4+: handlers backend para rule_change / membership / budget (que no están en el switch de finalize_vote todavía).
