...
- [~] V2-G2 Decision types con outcome handlers — sub-slices 1+2+3+4: propose surface + read surface + reference picker para sanction/mandate/membership (con target_state picker) + backend handler para `reference_kind='membership'` en `finalize_vote` (mig 20260528030000 agrega `p_metadata` a `start_vote` y la rama membership que lee `metadata.target_state` y llama `set_membership_state`). iOS DecisionsStore deriva el metadata desde draftMembershipTargetState. Falta sub-slice 5+: handlers backend para `rule_change` (mutar group_rules) y `budget` (crear obligations/pool_charges).
