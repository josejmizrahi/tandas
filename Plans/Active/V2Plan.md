...
- [~] V2-G2 Decision types con outcome handlers — sub-slices 1+2+3+4+5: propose surface + read surface + reference picker para sanction / mandate / membership / rule_change (con action picker archive/activate). Backend handlers: mig 20260528030000 (membership+metadata) + mig 20260528040000 (rule_change inline: archive→status='archived' + effective_until=now() en current_version; activate→status='active' cuando estaba archived/draft). iOS metadata composer cubre membership.target_state y rule.action. Falta sub-slice 6: handler backend para `budget` (crear obligations/pool_charges) y eventualmente `rule_change` con action='edit' (cambiar body/title — requiere meta.title/body adicionales).
