...
- [~] V2-G2 Decision types con outcome handlers — sub-slices 1+2: propose surface con los 11 valores canónicos agrupados + read surface de reference+outcome en DecisionDetailView. Backend `finalize_vote` ya dispatcha handlers via reference_kind→sanction/dispute/mandate_grant/mandate_revoke/dissolution (verificado pg_get_functiondef); iOS ahora muestra "Afecta a…" con tap-to-route (DeepLinkRouter) y "Consecuencia" post-finalize cuando outcome=passed. Falta sub-slice 3+: propose-side wiring (picker de sanción/disputa/mandato al elegir el type correspondiente) + handlers backend para rule_change / membership / budget (que no están en el switch de finalize_vote todavía).
