...
>
> Estado backend al 2026-05-28: 337+ migrations, `v1.0.0-rc` cortado,
> **V2-G3 ENTERO CERRADO** (G3.1→G3.5 ✅; G3.6 diferido a V3). 5 migs
> nuevas: `20260528060000..060500`. 427/427 tests RuulCore verdes.
> Engine viviendo end-to-end: 4 callsites cableados (`issue_sanction`,
> `set_membership_state`, `open_dispute`, `finalize_vote` — los 3 money
> RPCs venían cableados de antes). 3 handlers reales (sync sanction +
> sync membership + async notification). iOS Disparos feed accesible
> desde RulesListView toolbar. Banner global `GroupHomeFeedView`
> pendiente (no G3 retroactivo; cae en V2-G8). Restante V2: G8 +
> G4 (sanction ↔ money deep).
>
...
## Estado post-V2-G3 cierre completo (commits `30efadce`→`cf9a8b8d`)
✅ G3.1 → G3.5 todas cerradas. 5 migs (`060000`→`060500`) en `supabase/migrations/`. 427/427 tests RuulCore. 7 callsites cableados (3 pre + 4 nuevos). 3 predicates + 3 consequence handlers vivos. iOS Disparos feed accesible desde RulesListView toolbar.

**Pendiente menor** (no bloquea cierre G3):
- Banner global "Sistema evaluó M reglas en últimas 24h" en `GroupHomeFeedView` — pase a V2-G8.
- Founders todavía no publican reglas para `sanction.issued`, `member.state_changed`, `dispute.opened`, `decision.finalized` → esos callsites quedan audit-only hasta que existan rule fixtures reales.

## Siguiente foco (V2 restante)
1. **V2-G8 Engine-driven UX** (1-2 sesiones) — surface banner + per-resource "qué reglas matchearon aquí".
2. **V2-G4 Sanction ↔ Money deep** (3-4 sesiones) — partial payments, payment plans, auto-pay from fund. Independent de G8.

Tras G4+G8 → tag `v2.0.0-rc` → pase V3 (consolidación / cleanup / launch).
