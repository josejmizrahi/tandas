...
>
> Estado backend al 2026-05-28: 331 migrations, `v1.0.0-rc` cortado,
> V2 en 7/9 + **G3.1 ✅ DONE** (commit `04976c71`). `RuleShape.swift`
> vive en `Domain/`. `rule_shapes_catalog` con 9 atoms seedeados +
> 4 RPCs (`list_rule_shapes`, `validate_rule_shape`, `create_engine_rule`,
> `group_rules_engine`). 419/419 tests verdes. Pendiente G3.2-G3.5
> (con G3.4 antes/merged con G3.2 por doctrina lock-in #4).
>
...
**Why global**: reglas sin transparencia ni autorización catalogada erosionan confianza institucional. El engine es lo que diferencia Ruul de un Slackbot.
...
**9 atoms seedeados en G3.1** (3+3+3):
- Triggers (3): `trigger.expense.recorded`, `trigger.settlement.completed`, `trigger.membership.left` — cada uno con `schema.scope` declarado.
- Conditions (3): `condition.amount_above`, `condition.actor_role_is`, `condition.target_self` — `schema.fields` declarados.
- Consequences (3): `consequence.issue_sanction` (sync, `authority_required: sanctions.create`), `consequence.send_notification` (async, no authority), `consequence.set_membership_state` (sync, `authority_required: members.suspend`).

**4 RPCs vivas en backend post-G3.1**:
- `list_rule_shapes()` — iOS lee al boot.
- `validate_rule_shape(p_shape jsonb)` — dry-run validator.
- `create_engine_rule(...)` — atomic propose+publish, valida shape server-side.
- `group_rules_engine(p_group_id)` — lectura per-rule con shape + condition_tree + consequences.
...
### F.1.4 `group_rule_evaluations` (schema minimal vive; G3.3 ALTERs mandatorios)

Estado actual (post-G3.1):
```sql
id uuid PRIMARY KEY,
...
**ALTER mandatorio en G3.3** (lock-in #5 founder — no negociable):
```sql
ALTER TABLE group_rule_evaluations
  ADD COLUMN IF NOT EXISTS rule_id uuid REFERENCES group_rules(id),
  ADD COLUMN IF NOT EXISTS parent_evaluation_id uuid REFERENCES group_rule_evaluations(id),
  ADD COLUMN IF NOT EXISTS depth int NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS verdict text CHECK (verdict IN (
    'matched_consequences','matched_no_action','exception_short_circuit',
    'no_match','error','recursion_aborted','cycle_aborted'
  )),
  ADD COLUMN IF NOT EXISTS matched_predicate jsonb,   -- {passed, reason, evaluated_value, predicate_kind}
  ADD COLUMN IF NOT EXISTS actions_emitted jsonb,     -- [{kind, execution, status, target_id?, error?, ledger_entry_id?, sanction_id?, outbox_id?}]
  ADD COLUMN IF NOT EXISTS errors jsonb,
  ADD CONSTRAINT depth_max_5 CHECK (depth >= 0 AND depth <= 5);

-- Atom guard (append-only)
CREATE TRIGGER group_rule_evaluations_atom_guard
BEFORE UPDATE OR DELETE ON group_rule_evaluations
FOR EACH ROW EXECUTE FUNCTION ruul.raise_immutable_atom();
```

`matched_predicate` y `actions_emitted` son **load-bearing para G3.5 UX** (RuleEvaluationsView debe responder "qué pasó y por qué"). Sin estas columnas no se puede debuggear ni dar transparencia.
...
## F.4 Lifecycle integration con RPCs domain
...
BEGIN
  ...
  -- 3. INSERT INTO system_events (atomic event) RETURNING id INTO v_event_id
  -- 4. PERFORM evaluate_rules_for_event(v_event_id, 'sync', NULL, 0)
  -- 5. RETURN result
END;
```
...
## M.1 Prioridad ordenada (revisada 2026-05-28 post-G3.1 commit `04976c71`)

| # | Item | Capa | Sesiones | Estado / Bloquea |
|---|---|---|---|---|
| 0 | V2-G3.1 Shape catalog + builder + dry-run + 4 RPCs + iOS engine UX | Rules+iOS | — | ✅ DONE (commit `04976c71`, 419/419 tests) |
| 1 | **V2-G3.4 + G3.2 combinado** — handlers reales (issue_sanction sync, send_notification async, set_membership_state sync) + wire hook en 8 RPCs domain | Rules+Sanctions+Membership+Notif | 1-2 | doctrina lock-in #4: NO abrir G3.2 sin G3.4 |
| 2 | **V2-G3.3 ALTER `group_rule_evaluations`** — agregar `parent_evaluation_id`, `depth`, `matched_predicate jsonb`, `actions_emitted jsonb`, `verdict`, `errors`, atom guard, índices | Rules | 1 | bloquea G3.5 (explainability lock-in #5) |
| 3 | V2-G3.5 iOS RuleEvaluationsView (per-rule "Disparos" tab + banner global) | iOS UI | 1 | depende de G3.3 columns |
| 4 | ~~V2-G3.6 Simulator~~ → **diferido a V3** (dry-run ya quedó en G3.1) | Rules | — | V3 |
| 5 | V2-G8 Engine-driven UX (visible consequences en home + per-resource) | iOS UI | 1-2 | depende de G3 cerrado |
| 6 | V2-G4 Sanction ↔ Money deep (partial pay, plans, auto-pay from fund) | Money+Sanctions | 3-4 | independent |
| 7 | V3: Particionar `system_events` + `ledger_entries` monthly | Memory+Money | 2 | scalability |
| 8 | V3: `group_governance_versions` + `group_role_versions` snapshots | Authority+Governance | 1-2 | audit |
| 9 | V3: Deprecate `rules`, `fines`, `events` legacy tables | Cleanup | 1 | nothing critical |
| 10 | V3: Cron `discrepancy_alert` for ledger reconciliation | Money | 1 | observability |
| 11 | V3: Replay engine + freeform jsonb rule editor + simulator full | Memory+Rules | 2-3 | audit complete |
| 12 | V4: Subgroups (`groups.parent_group_id`) | Group | 5+ | org complex |
| 13 | V4: AI agents (`actor_kind='ai_agent'`) | Automation | 5+ | productizar |
...
## Cambios mandatorios inmediatos (V2-G3 restante post-`04976c71`)
- **G3.4 + G3.2 (merged)**: implementar `ruul.dispatch_consequences` + 3 handlers (issue_sanction sync, send_notification async, set_membership_state sync) + wire SYNC `evaluate_rules_for_event` hook en 8 RPCs domain críticas. **NO separar** (doctrina lock-in #4).
- **G3.3**: ALTER `group_rule_evaluations` con `parent_evaluation_id` + `depth` + `matched_predicate jsonb` + `actions_emitted jsonb` + `verdict` + `errors` + atom guard. **Columns explainability no negociables** (lock-in #5).
- **G3.5**: iOS RuleEvaluationsView consume las nuevas columns.
- Smoke tests N.4 + N.11.1 obligatorios antes de cerrar G3.
