...
16. 🟢 R.6 Rule Engine 2.0 implementación — **A** ✅ `8703ee2a` idempotency + emit_attention sink. **B** ✅ `0de668db` auto-dispatch trigger (+ post-fixes `1f2f8b92`). **F** ✅ `1f2f8b92` 2 seeds demoables. **E** ✅ `d3df75d5` iOS Rules + RuleTrigger P0 fix. **C** ✅ `04b48c95` pg_cron + obligation.overdue. **C.2/3/4** ✅ `4e430667` 3 detectors más (document.expiring + reservation.starting_soon + right.expiring). **D** ✅ `584693d8` DSL closed-grammar validator (BEFORE trigger on rules, condition + consequence whitelist). Cron-tick catalog: 4 de 5 (maintenance.due deferred). **G** founder smoke device pendiente.
```
